...
# If in-band cables were loaded before user runs snapshot
are_inband_cables_loaded = False
...
blueos_flag = False
# True --> Sysinfo-snapshot is missing some critical debugging information
...
st_infiniband_devices, infiniband_devices = no_log_status_output(
...
sriov_version = "1.0.0"
sriov_exists = False
sriov_commands_collection = [
    "bridge fdb show dev p3p1",
    "ip link",
    "ip_link_show_devices",
    "lspci_vf"]
available_sriov_commands_collection = []
sriov_internal_files_collection = [
...
perf_status_dict = {}
perf_val_dict = {}
perf_external_files_collection = [
    ["mlnx_tune -r -i " + path + file_name, "mlnx_tune_r"]]
perf_samples = {}
bandwidth = {}
latency = {}
...
# If mtusb flag is true it runs "mst start" and then return it to the old
# status
mtusb_flag = False
...
# this should be invoked with before any release with --ibdiagnet -fw -p
# --pcie --check_fw
...
# pcie_debug_flag = True, means to run the tool only for collecting
# PCIE_Debug info
no_ib_flag = False
...
gpu_command_collection = [
    "nvidia-smi topo -m",
    "nvidia-smi",
    "lspci -tv |grep 'NVIDIA' -A7",
    "nvidia-smi -q -d clock",
    "nvidia-smi --format=csv --query-supported-clocks=gr,mem",
    "ib_write_bw -h | grep -i cuda",
    "modinfo nv_peer_mem",
    "/usr/local/cuda/extras/demo_suite/deviceQuery",
    "/etc/init.d/nv_peer_mem status",
    "/usr/local/cuda/extras/demo_suite/bandwidthTest --memory=pinned --mode=range --start=65536 --end=65011712 --increment=4194304 --device=all --htod",
...
commands_collection = [
    ...
    "time",
    "df -lh",
    "/opt/mellanox/ethtool/sbin/ethtool --version",
    "ethtool_version",
    "ethtool_all_interfaces",
    "fdisk -l",
    "hostname",
    "ibdev2netdev",
    "ibdev2pcidev",
    ...
    "ulimit -a",
    "uname",
    "yy_MLX_modules_parameters",
    "sysclass_IB_modules_parameters",
    "proc_net_bonding_files",
    "sys_class_net_files",
    "teamdctl_state",
    "teamdctl_state_view",
    "teamdctl_config_dump",
    "teamdctl_config_dump_actual",
    "teamdctl_config_dump_noports",
    "mlxconfig_query",
    "mst status",
    "mst status -v",
    "mlxcables",
    ...
    "virsh list --all",
    "virsh vcpupin",
    "sys_class_infiniband_ib_paameters",
    "sys_class_net_ecn_ib",
    "roce counters",
    "route -n",
    "numastat -n",
    "NetworkManager --print-config",
    "networkManager_system_connections",
    "virsh version",
    "USER",
...
fabric_commands_collection = [
    "ib_mc_info_show",
    "sm_version",
    "Multicast_Information",
    "perfquery_cards_ports"]
fabric_multi_sub_commands_collection = [
    "ibdiagnet",
    "ib_find_bad_ports",
...
internal_files_collection = [
    ...
    "/etc/systemd/system/sysinit.target.wants/openibd.service",
...
external_files_collection = [["kernel config",
                              "/boot/config-$(uname -r)"],
                             ["mlxcables --DDM/--dump",
                              "cables/mlxcables_options_output"],
                             ["config.gz",
                              "/proc/config.gz"],
                             ["zoneinfo",
                              "/proc/zoneinfo"],
...
copy_under_files = [["etc_udev_rulesd", "/etc/udev/rules.d/"],
                    ["lib_udev_rulesd", "/lib/udev/rules.d/"]]
copy_openstack_dirs = [["conf_nova",
                        "/var/lib/config-data/puppet-generated/nova_libvirt"],
                       ["conf_nuetron",
                        "/var/lib/config-data/puppet-generated/neutron/"]]
copy_openstack_files = [["logs_nova",
...
running_warnings = []
# commands that are part of higher chain commands ,used when generating
# config file
sub_chain_commands = [
    "file: var/log/syslog",
    "file: var/log/messages",
    "file: var/log/boot.log",
    "mlnx_tune -i " + path + file_name,
    "ib_write_bw_test",
    "latency",
    "perf_samples",
    "mlxfwmanager --online-query-psid",
    "file: /sys/class/infiniband/*/iov",
    "file: /sys/class/infiniband/*/device/"]
...
# *****************************************************************************************************
#                                         log
# A decorater that helps figure out the timing each command took, parent
# function, input / output of the invoked command
...
def log(f):
    def wrap(*args):
        ...
        # parent function who called the command
...
if (os_st == 0 and cur_os != "%{_vendor}"):
    if (cur_os not in supported_os_collection):
        print("Operating system with vendor " + cur_os +
              " is not tested in this Linux sysinfo snapshot.")
        decide()
else:
    ...
    if (
        (
            "red" in o_systems and "hat" in o_systems) or (
            "redhat" in o_systems) or (
                "centos" in o_systems) or (
                    "fedora" in o_systems) or (
                        "scientific" in o_systems) or (
                            "yocto" in o_systems) or (
                                "blueos" in o_systems)):
        cur_os = "redhat"
...
# Runs ibswitches for a given card and port and returns it output and run
# status


def get_ibswitches_output(card, port):
    ibswitches_st, ibswitches = get_status_output(
        "/usr/sbin/ibswitches -C " + card + " -P " + port)
    if ibswitches_st != 0:
...
def get_installed_cards_ports():
    ...
    st, ibstat = get_status_output(
        "ibstat | grep " + '"' + "CA '\\|Port " + '"' + " | grep -v GUID")
    if st != 0:
                ...
                if len(str_ports) > 0:
                    ...
                    if len(str_ports) > 0:
                        for port in str_ports:
                            ...
                            st, ibnetdiscover_output = get_status_output(
                                "/usr/sbin/ibnetdiscover -C " + card_name + " -P " + port_num + "")
                            if st != 0:
                                continue
                            ibnetdiscover_output_hashed = hashlib.sha256(
                                ibnetdiscover_output.encode('utf-8')).hexdigest()
                            if ibnetdiscover_output_hashed not in all_sm_on_fabric:
                                all_sm_on_fabric.append(
                                    ibnetdiscover_output_hashed)
                                if card_name not in active_subnets:
                                    active_subnets[card_name] = []
                                obj = {}
                                obj["port_num"] = port_num
                                ibswitches_st, ibswitches = get_ibswitches_output(
                                    card_name, port_num)
                                obj["ibswitches"] = {}
...
class mlnx_device:
    ...
    def get_mlnx_device_info(self):
        ...
        res += "System Image GUID: " + "/sys/class/infiniband/" + \
            self.parent_device + "/sys_image_guid = " + self.sys_img_guid + "\n"
        res += "Board ID: " + "/sys/class/infiniband/" + \
            self.parent_device + "/board_id = " + self.board_id + "\n"
        res += "Firmware Version: " + "/sys/class/infiniband/" + \
            self.parent_device + "/fw_ver = " + self.fw_ver + "\n"
        res += "HCA Type: " + "/sys/class/infiniband/" + \
            self.parent_device + "/hca_type = " + self.hca_type + "\n"
        res += "HW Rev: " + "/sys/class/infiniband/" + \
            self.parent_device + "/hw_rev = " + self.hw_rev + "\n"
        res += "Node Type: " + "/sys/class/infiniband/" + \
            self.parent_device + "/node_type = " + self.node_type + "\n"
        for node_guid in self.node_guids:
            res += "\nNumber of related nodes with Node GUID = " + node_guid + " is: " + \
                str(len(self.node_guids[node_guid])) + ", with the following Node Descriptions: \n"
            for node_desc in self.node_guids[node_guid]:
                res += "  " + node_desc + "\n"
        return res

# return true if command should be invoked. and added to dict in case we
# are generating new file
        ...
        if config_key not in config_dict:
            ...
            if related_flag:
                ...
                for flag in related_flags:
                    if "no" not in flag.lower():
                        flag = flag.strip()
                        approved = "no"

            config_dict[config_key] = {
                "approved": approved,
                "related flag": related_flag}
        return False

    if config_key in config_dict:
        if config_file_flag and config_dict[config_key]["approved"].lower(
        ) != 'yes':
            return False
        else:
            return True
        ...
        if config_file_flag:
            print(
                "Warning: following command not collected it's missing in config file :\n" +
                config_key +
                ' \n Please make sure that you are using sysinfo-snapshot version (' +
                version +
                ') to generate config file (config.csv)  \n')
            running_warnings.append(
                "following command not collected it's missing in config file :\n" +
                config_key +
                "\n")
    return True
...
def update_net_devices():
    ...
    if os.path.isdir('/dev/mst'):
        ...
        for device in current_mst_devices:
            if (not (device.startswith("CA") or device.startswith("SW"))
                    and "cable" in device):  # Make sure we get info only for local cables
                local_mst_devices.append(device)

    if not sys_class_net_exists:
        errors.append(
            "No Net Devices - The path /sys/class/net does not exist")
    ...
    if (st != 0):
        ...
        all_net_devices = network_devices.splitlines()

    # e.g 81:00.0 Infiniband controller: Mellanox Technologies MT27800 Family
    # [ConnectX-5]
    ...
    for lspci_device in pci_devices:
        ...
        if "function" in lspci_device.lower():
            if device not in vf_pf_devices:
                vf_pf_devices.append(device)
        # e.g lrwxrwxrwx  1 root root 0 Oct 26 15:51 ens11f0 ->
        # ../../devices/pci0000:80/0000:80:03.0/0000:81:00.0/net/ens11f0
...
if (is_ib == 0 and no_ib_flag == False):
    l3_dict[str(section_count) +
            ". Fabric Diagnostics Information: "] = fabric_commands_dict
    section_count += 1
        ...
        for root, dirs, files in os.walk('/sys/class/infiniband'):
            for device in dirs:
                if os.path.isdir('/sys/class/infiniband/' + device + '/ports'):
                    for subroot, subdirs, subfiles in os.walk(
                            '/sys/class/infiniband/' + device + '/ports'):
                        for port in subdirs:
                            if os.path.isdir(
                                    '/sys/class/infiniband/' + device + '/ports/' + port + '/gids'):
                                for subsubroot, subsubdirs, subsubfiles in os.walk(
                                        '/sys/class/infiniband/' + device + '/ports/' + port + '/gids'):
                                    for gid_index in subsubfiles:
                                        ...
                                        if gid.split(':')[0] == '0000':
                                            try:
                                                ipv4 = str(int(gid[30:32], 16)) + '.' + str(int(gid[32:34], 16)) + '.' + str(
                                                    int(gid[35:37], 16)) + '.' + str(int(gid[37:39], 16)) + '  \t'
                                            except BaseException:
                                                ipv4 = '\t\t'
                                            res += device + '\t' + port + '\t' + gid_index + '\t' + \
                                                gid + '\t\t' + ipv4 + gid_type + '\t\t' + gid_ndevs + '\n'
                                        else:
                                            res += device + '\t' + port + '\t' + gid_index + '\t' + \
                                                gid + '\t\t\t\t' + gid_type + '\t\t' + gid_ndevs + '\n'
...
def ethtool_version_handler():
    ...
    if st != 0:
        ...
        st, ethtool_version = get_status_output(
            ethtool_command_2 + " --version")
        if st != 0:
            return st, "Failed to run the command " + \
                ethtool_command + " or " + ethtool_command_2
...
def ethtool_all_interfaces_handler():
    ...
    for interface in mellanox_net_devices:
        ...
        for option in options:
            st, ethtool_interface = get_status_output(
                ethtool_command + " " + option + " " + interface)
...
def ethtool_S_output(mellanox_net_devices):
    ...
    for interface in mellanox_net_devices:
        ...
        file = open(
            path +
            file_name +
            "/ethtool_S/ethtool_S_" +
            filtered_interface_name,
            'w')
        file.write(ethtool_interface)
        file.close()
        res += "ethtool -S " + interface + "\n"
        res += "<td><a href=ethtool_S/ethtool_S_" + \
            filtered_interface_name + ">ethtool -S " + interface + "</a></td>"
        res += "\n\n--------------------------------------------------"
    return res
# **********************************************************
#        modinfo Handler
...
def modinfo_handler():
    ...
    for module in modules:
        ...
        st, modinfo_module = get_status_output(
            "modinfo " + module + " | grep 'filename\\|version:'")
        if (st != 0):
            modinfo_module = "Could not run: " + '"' + " modinfo " + \
                module + " | grep 'filename\\|version:'"
...
def devlink_handler():
    ...
    for device in pci_devices:
        for i, reporter in enumerate(devlink_health_json[device]):
            filtered_device_name = device.replace(
                ":",
                "").replace(
                ".",
                "").replace(
            ...
            if 'name' in reporter.keys():
                ...
                result += '\nError in parsing ' + device + \
                    'information from: "devlink health show -j"\n'
                continue
            if reporter[reporter_key] == "fw_fatal" and 'last_dump_time' in devlink_health_json[device][i].keys():
                command = "devlink health dump show %s reporter %s " % (
                    device, reporter[reporter_key])
                ...
                dump_output_st, dump_output = get_status_output(
                    "devlink region dump " + device + "/" + space + " snapshot " + snapshot_id)
                if (dump_output_st != 0):
                    dump_output_result += "Error while reading output from command - " + command + "\n"
                dump_output_result += dump_output
                devlink_file_name = filtered_device_name + "_" + \
                    option.replace(" ", "_") + "_" + reporter[reporter_key] + ".txt"
                full_file_name = "devlink/devlink_" + devlink_file_name
                file = open(path + file_name + "/" + full_file_name, 'w+')
                file.write(dump_output_result)
                file.close()
                result += "<td><a href=" + full_file_name + \
                    "> " + devlink_file_name + "</a></td>"
                result += "\n--------------------------------------------------\n"
            else:
                for option in options:
                    if 'last_dump_time' in devlink_health_json[device][i].keys() or (
                            not option == "dump show"):
                        command = "devlink health %s %s reporter %s " % (
                            option, device, reporter[reporter_key])
                        dump_output_result = command + "\n\n"
                        dump_output_st, dump_output = get_status_output(
                            command)
                        if (dump_output_st != 0):
                        ...
                        full_file_name = "devlink/devlink_" + devlink_file_name
                        file = open(
                            path + file_name + "/" + full_file_name, 'w+')
                        file.write(dump_output_result)
                        file.close()
                        # dump_output_result += "<td><a href=" + full_file_name
                        # + "> " file_name + "</a></td>"
                        result += "<td><a href=" + full_file_name + \
                            "> " + devlink_file_name + "</a></td>"
                        result += "\n--------------------------------------------------\n"
...
def mlnx_qos_handler():
    ...
    for interface in mellanox_net_devices:
        res += "\n\n"
        for option in options:
            st, mlnx_qos_interface = get_status_output(
                "mlnx_qos " + option + " " + interface)
...
def cma_roce_handler(func):
    ...
    for device in devices.splitlines():
        ...
        st, _device_res = get_status_output(
            "cma_roce_" + func + " -d " + _device)
        if not first:
            res += "\n\n---------------\n\n"
        ...
        if "mtusb" not in device:
            for option in options:
                ...
                filtered_file_name = output.replace(
                    ...
                    "/",
                    "_")
                st, res = get_status_output("mlxdump -d " +
                                            device +
                                            " " +
                                            option +
                                            " --gvmi " +
...
def add_mlxdump_links():
    ...
    for file in os.listdir(path + file_name + "/firmware"):
        if file.startswith("mlxdump"):
            file_link[file] = "<td><a href=firmware/" + \
                file + ">" + file + "</a></td>"
    return file_link
...
def asap_handler():
    ...
    try:
        with open(path + file_name + "/asap/ovs_dpctl_dump_flows", "w+") as outF:
            ...
            st, res = get_status_output(
                "ovs-dpctl dump-flows -m >> " + path + file_name + "/asap/ovs_dpctl_dump_flows", '300s')
            if st != 0:
                outF.write("Could not run: ovs-dpctl dump-flows -m")
            else:
                is_all_failed = False
        result.append(
            "<td><a href=asap/ovs_dpctl_dump_flows> ovs_dpctl_dump_flows </a></td>")
    except BaseException:
    ...
    try:
        with open(path + file_name + "/asap/tc_qdisc_show", "w+") as outF:
            ...
            st, res = get_status_output(
                "tc qdisc show >> " + path + file_name + "/asap/tc_qdisc_show", '300s')
            if st != 0:
                outF.write("Could not run: ovs-dpctl tc qdisc show")
            else:
                is_all_failed = False
        result.append(
            "<td><a href=asap/tc_qdisc_show> tc_qdisc_show </a></td>")
    except BaseException:
    ...
    try:
        with open(path + file_name + "/asap/ovs-vsctl_get_Open_vSwitch", "w+") as outF:
            ...
            st, res = get_status_output("ovs-vsctl get Open_vSwitch . other_config >> " +
                                        path + file_name + "/asap/ovs-vsctl_get_Open_vSwitch", '300s')
            if st != 0:
                outF.write(
                    "Could not run: ovs-vsctl get Open_vSwitch . other_config")
            else:
                is_all_failed = False
        result.append(
            "<td><a href=asap/ovs-vsctl_get_Open_vSwitch> ovs-vsctl get Open_vSwitch </a></td>")
    except BaseException:
        err = "Could not open the file: " + file_name + \
            "/asap/ovs-vsctl_get_Open_vSwitch."
        running_warnings.append(err)

    st, output = get_status_output(
        "ovs-vsctl show | grep Bridge | awk '{$1=$1};1' | cut -d' ' -f 2", '300s')
    if "command not found" in str(output) or st != 0:
        try:
            with open(path + file_name + "/asap/ovs_dpctl_dump_flows_bridges", "a+") as outF:
                outF.write(
                    "Could not run:ovs-vsctl show | grep Bridge | awk '{$1=$1};1' | cut -d' ' -f 2")
        except BaseException:
            err = "Could not open the file: " + file_name + \
                "/asap/ovs_dpctl_dump_flows_bridges."
            running_warnings.append(err)
    else:
            ...
            if 'b' in row:
                ...
                try:
                    with open(path + file_name + "/asap/ovs_dpctl_dump_flows_bridges", "a+") as outF:
                        outF.write(cmd)
                        st, res = get_status_output(
                            cmd + " >> " + path + file_name + "/asap/ovs_dpctl_dump_flows_bridges", '300s')
                        if st != 0:
                    ...
                    running_warnings.append(err)
    result.append(
        "<td><a href=asap/ovs_dpctl_dump_flows_bridges> ovs_dpctl_dump_flows_bridges </a></td>")
    if is_all_failed:
...
def asap_tc_handler():
    if not asap_devices:
        running_warnings.append(
            "--asap_tc flag was provided but there's no interfaces found.")
    ...
    for interface in asap_devices:
        ...
        try:
            with open(path + file_name + "/asap_tc/ovs_tc_filter_" + interface, "a+") as outF:
                outF.write(cmd)
                st, res = get_status_output(
                    cmd + " >> " + path + file_name + "/asap_tc/ovs_tc_filter_" + interface, '300s')
                if st != 0:
                    outF.write("Could not run: " + cmd)
                else:
                    is_all_failed = False
            result.append(
                "<td><a href=asap_tc/ovs_tc_filter_" +
                interface +
                "> ovs_tc_filter_" +
                interface +
                " </a></td>")
...
def rdma_tool_handler():
    ...
    for option in options:
        with open(path + file_name + "/rdma_tool/" + option, "a+") as outF:
            ...
            st, res = get_status_output(
                "/opt/mellanox/iproute2/sbin/rdma " + option)
            if st == 0:
                outF.write(res)
                is_all_failed = False
            else:
                outF.write(
                    "Could not run: /opt/mellanox/iproute2/sbin/rdma " +
                    option)
        result.append(
            "<td><a href='rdma_tool/" +
            option +
...
def ibdev2pcidev_handler():
    ...
    final_mapping = ""
    for device in devices.splitlines():
        # Example output:  /sys/class/infiniband/mlx5_3/device ->
        # ../../../0000:8b:00.1
        ...
        if st2 != 0:
            ...
            # Example: mlx5_0 is mapped to /sys/class/infiniband/mlx5_0/device
            # -> ../../../0000:86:00.0
            device_mapping_split = device_mapping.split("/")
            # Now it will be mlx5_0 --> 0000:86:00.0 (The last part only)
            device_pci_mapping = device_mapping_split[-1]
...
def fwtrace_handler():
    ...
    for pci_device in pci_devices:
        ...
        for option in options:
            ...
            fwtrace_st, fwtrace_device_option = get_status_output(
                "fwtrace -d " + device + " " + option, '2m')
            if (fwtrace_st != 0):
...
def mlxcables_options_handler():
    ...
    for mlxcable in mlxcables:
        ...
        for option in options:
            ...
            res_st, res_mlxcable_option = get_status_output(
                'mlxcables -d ' + mlxcable + ' ' + option)
            if res_st != 0:
                res_mlxcable_option = 'Could not run: \"mlxcables -d ' + \
                    mlxcable + ' ' + option + '"'
    ...
    if os.path.isdir(path + file_name + "/cables"):
        try:
            ...
            res_mlxcable_option = "Could not open the file: " + \
                file_name + "/cables/mlxcables_options_output"
            res += res_mlxcable_option
    else:
...
def mlxcables_standard_handler():
    ...
    if with_inband_flag:  # If in-band cable info is requested, we have to load in-band cables anyways
        ...
        f.write(res)
        f.close()
        mlxcables_out.append(
            "<td><a href=\"cables/mlxcables_output\">mlxcables_output</a></td>")
        ...
        if are_inband_cables_loaded:
            ...
            for device in current_mst_devices:  # Run mlxcables only on local cables even if in-band cables are loaded
                if (not (device.startswith("CA") or device.startswith("SW"))
                    ...
                    mlxcables_res += "\n"
                    mlxcables_res += res
            # To put output in /cables, but still display it in HTML file
            # because --no_inband was NOT given
...
def lspci_vv_handler():
    # A list containing (first part of) PCI addresses of Mellanox devices
    mlnx_pci = []
    ...
    # Iterate over each mlnx_pci address, and check if it's linked in the pci
    # tree. If it is, add it to invoke later.
    for mlx_pci in mlnx_pci:
        # Need to go over each line of the pci_links paragraph, and if MLNX PCI
        # is there, it means they're linked
        for index in range(1, len(split_pci_tree)):
            split_pci_tree_lines = split_pci_tree[index].splitlines()
            for line in split_pci_tree_lines:
                if ('[' + mlx_pci + ']') in line:
                    second_part = line.split(('[' + mlx_pci + ']'))[0]
                    # Clean the string to get the correct second part of the
                    # final PCI address to invoke
                    second_part = re.sub('[^\\d\\.]', '', second_part)
                    first_part = split_pci_tree[index -
                                                1].split(':')[1].replace(']', '')
...
# **********************************************************
# Helper for mstcommand_d_handler - handles commands with given number of
# runs


def command_with_number_of_runs(
        number_of_runs,
        device,
        command,
        suffix,
    ...
    for i in range(0, number_of_runs):
        if "--amber_collect" in suffix:
            if pci_device:
                suffix = " --amber_collect " + path + file_name + "/amber_info/amber_collect_" + \
                    str(i + 1) + "_" + str(device) + ".csv --port_type PCIE"
            else:
                suffix = " --amber_collect " + path + file_name + \
                    "/amber_info/amber_collect_" + str(i + 1) + "_" + str(device) + ".csv"
        command_result += "\n#" + \
            str(i + 1) + " " + command + " -d " + device + suffix + "\n\n"
        mlx_st, command_result_device = get_status_output(
            command + " -d " + device + suffix, "30s")
        if pcie_debug:
            output = command + "_" + device + "_run_" + str(i + 1)
            filtered_file_name = output.replace(":", "").replace(".", "")
            save_mlxlink_output_to_file(
                filtered_file_name, command_result_device)
        command_result_device = command_result_device.replace(
            "[31m", "").replace(
            "[32m", "").replace(
            "[33m", "").replace(
                "[0m", "")
        if (mlx_st != 0):
            command_result_device = "Errors detected while running: " + \
                command + " -d " + device + suffix + '"\n' + command_result_device
...
def mstcommand_d_handler(command, pcie_debug=False):
    ...
    if (command == 'mlxlink / mstlink'):
        if mft_installed:
            # If MFT is installed but it is an old version that does NOT
            # include Mlxlink
            st, mlxlink_test = get_status_output('man mlxlink')
            if st == 0:
                command = 'mlxlink'
    ...
    for local_mst_device in local_mst_devices:
        command_result += command_with_number_of_runs(
            number_of_runs, local_mst_device, command, suffix, pcie_debug)
    if mtusb_flag:
        for mtusb_device in mtusb_devices:
            command_result += command_with_number_of_runs(
                number_of_runs, mtusb_device, command, suffix, pcie_debug)
    # PCIe ports
    for pci_device in pci_devices:
        device = pci_device["device"]
        if mft_installed:
        ...
        if "cable" in device:
            continue
        if command == "mlxmcg" and not (
                "ConnectX-3" in pci_device["name"] or "ConnectX-3 Pro" in pci_device["name"]):
            continue
        for suffix in suffix_list:
            ...
            # PCIe ports
            command_result += command_with_number_of_runs(
                number_of_runs, device, command, suffix, pcie_debug, True)
...
def general_fw_command_output(command, card, timeout='80s'):
    ...
    mlxdump_notsupported_error = "Device not supported, couldn't run mlxdump " + \
        card + " pcie_uc --all"
    ...
    commands_dict['fwconfig'] = [
        "mstconfig -d " + card + " -e  q",
        "mlxconfig -d " + card + " -e  q"]
    ...
    if command == 'fwflint_q' or command == 'fwflint_dc':
        ...
        if command == 'fwflint_q':
            ...
            flint_flags = ' dc'
        st, fw_query = get_status_output(
            commands_dict["fwflint"][0] + flint_flags, timeout)
        if st == 0:
            return (0, fw_query, 'mst')
        else:
            st, fw_query = get_status_output(
                commands_dict["fwflint"][1] + flint_flags, timeout)
            if st == 0:
        ...
        if st == 0:
            ...
            st, fw_query = get_status_output(
                commands_dict['fwdump'][1], timeout)
            if st == 0:
                return (0, fw_query, 'mft')
            else:
                return (1, command_error, tool_error)
        ...
        if st == 0:
            ...
            st, fw_query = get_status_output(
                commands_dict['fwconfig'][1], timeout)
            if st == 0:
                return (0, fw_query, 'mft')
            return (1, command_error, tool_error)
...
def general_fw_commands_handler(
        command,
        card,
        filtered_file_name,
        timeout='80s'):
    if command == 'fwdump':
        ...
        if tool_used == 'mst':
            try:
                ...
                with open(path + file_name + "/err_messages/dummy_functions", 'a') as f:
                    ...
                    f.write("\n\n")
                    return ("Error in creating new file in the system",
                            "Error in creating new file in the system", 1)
        elif tool_used == 'mft':
            try:
                f = open(
                    path +
        ...
        if tool_used == 'mst':
            try:
                ...
                with open(path + file_name + "/err_messages/dummy_functions", 'a') as f:
                    f.write(
                        "The full command is: mstconfig -d " +
                        card +
                        " -e  q" +
                        "\n")
                    f.write("Error in creating new file in the system")
            ...
            try:
                ...
                with open(path + file_name + "/err_messages/dummy_functions", 'a') as f:
                    f.write(
                        "The full command is: mlxconfig -d " +
                        card +
                        " -e  q" +
                        "\n")
                    f.write("Error in creating new file in the system")
        ...
        if tool_used == 'mst':
            try:
                ...
                with open(path + file_name + "/err_messages/dummy_functions", 'a') as f:
                    f.write(
                        "The full command is: mstflint -d " +
                        card +
                        flint_flags +
                        "\n")
                    f.write("Error in creating new file in the system")
            ...
            try:
                ...
                with open(path + file_name + "/err_messages/dummy_functions", 'a') as f:
                    f.write("The full command is: mstflint -d " +
                            card + flint_flags + flint_flags + "\n")
                    f.write("Error in creating new file in the system")
                    f.write("\n\n")
                    return ("Error in creating new file in the system",
                            "Error in creating new file in the system", 1)
...
def generate_card_logs(card, sleep_period, mstregdump_out, mst_status_output):
    ...
    filtered_file_name = output.replace(
        ...
        "").replace(
            "/",
        "")
    output_file, tool_used, is_error = general_fw_commands_handler(
        'fwconfig', card, filtered_file_name)
    if is_error == 0:
        if tool_used == 'mst':
    ...
    filtered_file_name = output.replace(
        ...
        "")
    output_file, tool_used, is_error = general_fw_commands_handler(
        'fwflint_q', card, filtered_file_name)
    if is_error == 0:
        if tool_used == 'mst':
            mstregdump_out.append(
                "<td><a href=\"" +
    ...
    filtered_file_name = output.replace(
        ...
        "")
    output_file, tool_used, is_error = general_fw_commands_handler(
        'fwflint_dc', card, filtered_file_name)
    if is_error == 0:
        if tool_used == 'mst':
            mstregdump_out.append(
                "<td><a href=\"" +
    ...
    filtered_file_name = output.replace(
        ...
        "")
    output_file, tool_used, is_error = general_fw_commands_handler(
        'mlxdump', card, filtered_file_name)
    if is_error == 0:
        mstregdump_out.append(
            "<td><a href=\"" +
            output_file +
...
def generate_mst_dumps(
    ...
    for i in range(0, 3):
        ...
        filtered_file_name = output.replace(
            ...
            "")
        output_file, tool_used, is_error = general_fw_commands_handler(
            'fwdump', card, filtered_file_name)
        if is_error == 0:
            if tool_used == 'mst':
                mstregdump_out.append(
                    "<td><a href=\"" +
...
def mst_func_handler():
    ...
    for card in all_devices:
        t_dumps = threading.Thread(
            target=generate_mst_dumps,
            args=[
                card,
                sleep_period,
                mstregdump_out,
    ...
    if mstregdump_out == []:
        mstregdump_out.append(
            "There was no mst query output from this devices\n")
        return 2, mstregdump_out
...
def mlnx_snap_handler():
    ...
    for file in mlnx_snap_files:
        st, content = get_status_output(
            "cp /etc/mlnx_snap/" + file + " " + path + file_name + "/mlnx_snap/" + file)
        res.append(
            "<td><a href=mlnx_snap/" +
            file +
            "> /etc/mlnx_snap/" +
    ...
    for interface in net_devices:
        ...
        try:
            with open(path + file_name + "/show_irq_affinity_all/" + interface, "w+") as outF:
                ...
                st, show_irq_affinity = get_status_output(
            ...
            res.append(
                "<td><a href=show_irq_affinity_all/" +
                interface +
            ...
            with open(path + file_name + "/networkManager_system_connections/" + connection, "w+") as outF:
                ...
                if (st == 0 and content != ""):
                    ...
                    outF.write(
                        ...
                        " does not exist")
            res.append(
                "<td><a href=networkManager_system_connections/" +
                connection +
...
def add_txt_command_output(command, output):
    ...
    clean_file_name = forbidden_chars.sub(
        r'', command).replace(
        "\\", "")  # clean the file name
...
def sys_class_infiniband_handler():
    ...
    for device in ib_devices_lines:
        sys_img_guid = get_file_content(
            "/sys/class/infiniband/" + device + "/sys_image_guid")

        if not (sys_img_guid in sys_img_dict.keys()):
            parent_device = device
            board_id = get_file_content(
                '/sys/class/infiniband/' + device + '/board_id')
            fw_ver = get_file_content(
                '/sys/class/infiniband/' + device + '/fw_ver')
            hca_type = get_file_content(
                '/sys/class/infiniband/' + device + '/hca_type')
            hw_rev = get_file_content(
                '/sys/class/infiniband/' + device + '/hw_rev')
            node_type = get_file_content(
                '/sys/class/infiniband/' + device + '/node_type')
            node_desc = get_file_content(
                '/sys/class/infiniband/' + device + '/node_desc')
            sys_img_dict[sys_img_guid] = mlnx_device(
                parent_device,
                sys_img_guid,
                board_id,
                fw_ver,
                hca_type,
                hw_rev,
                node_type)  # Initialize mlnx_device object

        node_guid = get_file_content(
            '/sys/class/infiniband/' + device + '/node_guid')
        node_desc = get_file_content(
            '/sys/class/infiniband/' + device + '/node_desc')
        node_desc_output = 'Node Description: /sys/class/infiniband/' + \
        ...
        for indir in os.listdir("/sys/class/net/"):
            if (os.path.exists("/sys/class/net/" + indir + "/ecn")):
                ...
                if (not os.path.exists(path + file_name + "/ecn/config")):
                    no_log_status_output(
                        "mkdir " + path + file_name + "/ecn/config")
                for root, dirs, files in os.walk(
                        "/sys/class/net/" + indir + "/ecn"):
                    try:
        ...
        for indir in os.listdir("/sys/kernel/debug/mlx5/"):
            if (os.path.exists("/sys/kernel/debug/mlx5/" + indir + "/cc_params")):
                ...
                if (not os.path.exists(path + file_name + "/ecn/debug")):
                    no_log_status_output(
                        "mkdir " + path + file_name + "/ecn/debug")
                for root, dirs, files in os.walk(
                        "/sys/kernel/debug/mlx5/" + indir + "/cc_params"):
...
def nvsm_dump_health_handler():
    ...
    # the result of the command is a tar archive maybe have to use tar commad
    # annd append the archive contant as href
    st, result = get_status_output(
        "nvsm dump health -tfp " + path + file_name, "6m")
    if st != 0:
...
def mlxreg_handler():
    ...
    if mst_devices_exist:
        ...
        for device in mst_devices:
            if "cable" not in device:
                res += "mlxreg -d /dev/mst/" + device + " --reg_name ROCE_ACCL --get \n\n"
                st, result = get_status_output(
                    "mlxreg -d /dev/mst/" + device + " --reg_name ROCE_ACCL --get")
                if st == 0:
                    res += result + "\n\n"
                else:
                    if "command not found" in result:
                        return 1, "Could not run the command, mlxreg does not exist"
                    res += "Could not run the command,mlxreg -d /dev/mst/" + \
                        device + " --reg_name ROCE_ACCL --get\n\n"
                res += "---------------------------------------------------------\n\n"
        if not res:
...
def virsh_vcpupin_handler():
    ...
    if st != 0:
        return 1, "Could not run 'virsh' command"
    if 'running' not in virsh_list_all:
...
def roce_counters_handler():
    ...
    for ib_device in ib_devices:
        st, ib_device_hw_counter_files = get_status_output(
            "ls /sys/class/infiniband/" + ib_device + "/hw_counters")
        if st == 0:
            ...
            for ib_device_hw_counter_file in ib_device_hw_counter_files:
                file_content = get_file_content(
                    ...
                    ib_device +
                    "/hw_counters/" +
                    ib_device_hw_counter_file +
                    "")
                roce_hw_counters += "/sys/class/infiniband/" + ib_device + "/hw_counters/" + \
                    ib_device_hw_counter_file + "" + ': ' + file_content + '\n'
        st, ports = get_status_output(
            "ls /sys/class/infiniband/" + ib_device + "/ports")
        if st != 0:
            return 1, "Couldn't get RoCE Counters - /sys/class/infiniband/" + \
                ib_device + "/ports does not exist."
        ports = ports.splitlines()
        for port in ports:
            st, hw_counter_files = get_status_output(
                "ls /sys/class/infiniband/" + ib_device + "/ports/" + port + "/hw_counters")
            if st != 0:
                roce_hw_counters += "RoCE HW Counters for " + \
                    ib_device + " port: " + port + " do not exist.\n"
            else:
                ...
                for hw_counter_file in hw_counter_files:
                    file_content = get_file_content(
                        ...
                        hw_counter_file +
                        "")
                    roce_hw_counters += "/sys/class/infiniband/" + ib_device + "/ports/" + \
                        port + "/hw_counters/" + hw_counter_file + "" + ': ' + file_content + '\n'
            st, counters_files = get_status_output(
                "ls /sys/class/infiniband/" + ib_device + "/ports/" + port + "/counters")
            if st != 0:
                roce_counters += "RoCE Counters for " + \
                    ib_device + " port: " + port + " do not exist.\n"
            ...
            for counters_file in counters_files:
                file_content = get_file_content(
                    ...
                    counters_file +
                    "")
                roce_counters += "/sys/class/infiniband/" + ib_device + "/ports/" + \
                    port + "/counters/" + counters_file + "" + ': ' + file_content + '\n'
...
def add_command_if_exists(command):
    ...
    if ((no_fw_flag) and (command in fw_collection)):
        return
    ...
    if ((fsdump_flag == False) and (command in fsdump_collection)):
        return
    if ((no_ib_flag) and (command in ib_collection)):
        return
    print_err_flag = 1
    # status = 0
    # invoke command reguarly if exists or redirect to the corresponding
    # function
    if (command == "date"):
        result = date_cmd
        status = 0
        ...
        if (os.path.isfile(ufm_snapshot_path)):
            status, result = get_status_output(
                ufm_snapshot_path + " -S all -e A -O " + path + file_name + "/ufm_logs")
            if (status == 0):
                print_err_flag = 0
        else:
            status = 1
        ...
        st, result = get_status_output(
            "awk '{ print FILENAME " + '"' + "=" + '"' + " $0  }' /sys/module/mlx*/parameters/*")
        if (st == 0):
            status = 0
            print_err_flag = 0
        else:
        ...
        print_err_flag = 0
        result = ""
        # e.g ip link ls type team #19: team0.100: <BROADCAST,MULTICAST> mtu
        # 1500 qdisc noop state DOWN mode DEFAULT qlen 1000
    ...
    # if iscsiadm --version command exists, add all isciadm commands to the
    # available ones
    ...
    if ((status == 0) or (command.startswith("service"))):
        ...
        if command_is_string:
            ...
            if "lspci" not in command:
                add_txt_command_output(command, result)
        else:
            available_commands_collection[not is_command_string].append(
                command)
    else:
        if (print_err_flag == 1):
            f = open(path + file_name + "/err_messages/dummy_functions", 'a')
...
def multicast_information_handler():
    ...
    st, MLIDS = get_status_output(
        "saquery -g | grep -i Mlid | sed 's/\\./ /g'|awk '{print $2}' | sort | uniq")
    if (st != 0):
    ...
    for MLID in MLIDS:
        st, saquery_mlid = get_status_output(
            "saquery -m " + MLID + " --smkey 1 2>/dev/null")
        if (st != 0):
            saquery_mlid = "Could not run: " + '"' + \
                "saquery -m " + MLID + " --smkey 1 2>/dev/null" + '"'
...
def perfquery_cards_ports_handler():
    ...
    for card in installed_cards_ports:
        ...
        for port in installed_cards_ports[card]:
            st, perfquery = get_status_output(
                "perfquery --Ca " + card + " --Port " + port)
            res += perfquery + "\n"
            first = False
    return res
...
def ib_find_bad_ports_handler(card, port):
    ...
    st, iblinkinfo_bad = get_status_output(
        "iblinkinfo --Ca " + card + " --Port " + port + "| grep Could")
    if st != 0:
...
def ib_find_disabled_ports_handler(card, port):
    ...
    st, iblinkinfo_disabled = get_status_output(
        "iblinkinfo --Ca " + card + " --Port " + port + "| grep Disabled")
    if st != 0:
...
def calc_IP(MGID):
    st, IP = get_status_output("ip=`echo " +
                               MGID +
                               " | awk ' { mgid=$1; n=split(mgid, a, " +
                               '"' +
                               ":" +
                               '"' +
                               "); upper=strtonum(" +
                               ...
                               "0x" +
                               '"' +
                               " a[n-1]); lower=strtonum(" +
                               '"' +
                               "0x" +
                               '"' +
                               " a[n]); addr=lshift(upper,16)+lower; addr=or(addr,0xe0000000); a1=and(addr,0xff); addr=rshift(addr,8); a2=and(addr,0xff); addr=rshift(addr,8); a3=and(addr,0xff); addr=rshift(addr,8); a4=and(addr,0xff); printf(" +
                               '"' +
                               "%u.%u.%u.%u" +
                               '"' +
                               ", a4, a3, a2, a1); }'`; echo $ip")
...
def ib_mc_info_show_handler():
    ...
    for index in range(0, len(saquery)):
        if "Mlid" in saquery[index]:
            ...
            if Mlid_val not in mlids_collection:
            ...
            if NodeDescription not in nodes_collection:
...
def ib_switches_FW_scan_handler(ibswitches_st, ibswitches, ibdiagnet_suffix):
    ...
    for ibswitch in ibswitches.splitlines():
        ...
        if ibdiagnet_is_invoked and not ibdiagnet_error:
            tmp_st, row = get_status_output("awk '/START_NODES_INFO/,/END_NODES_INFO/' " +
                                            path +
                                            file_name +
                                            "/" +
                                            ibdiagnet_suffix +
                                            "/ibdiagnet/ibdiagnet2.db_csv | grep ^" +
                                            guid)
            splt_row = re.split("[" + re.escape(",\n") + "]", row)
            if (1 < len(splt_row) and (represents_Int_base_16(splt_row[1]))):
                device_id = str(int(splt_row[1], 16))

            if (12 < len(splt_row)):
                fw_psid = splt_row[12]

            if (16 < len(splt_row) and (represents_Int_base_16(splt_row[14])) and (
                    represents_Int_base_16(splt_row[15])) and (represents_Int_base_16(splt_row[16]))):
                fw_version = str(int(splt_row[14],
                                     16)) + "." + str(int(splt_row[15],
                                                          16)) + "." + str(int(splt_row[16],
                                                                               16))

            if (7 < len(splt_row) and (represents_Int_base_16(splt_row[7]))):
                fw_build_id = str(int(splt_row[7], 16))

            if (2 < len(splt_row) and (represents_Int_base_16(splt_row[2]))):
...
def ib_topology_viewer_handler(card, port):
    ...
    suffix = card + "_" + port
    st, GUIDS = get_status_output("cat " +
                                  path +
                                  file_name +
                                  "/ibnetdiscover_p_" +
                                  suffix +
                                  " | grep -v -i sfb | grep -e ^SW | awk '{print $4}' | uniq")
    if (st != 0):
        return "Could not run: " + '"' + "cat " + path + file_name + "/ibnetdiscover_p_" + \
            suffix + " | grep -v -i sfb | grep -e ^SW | awk '{print $4}' | uniq" + '"'
    if (GUIDS == ""):
        return "No switches were found"

    GUIDS = sorted(GUIDS.split("\n"))
    ...
    for index in range(0, len(GUIDS)):
        ...
        st, desc = get_status_output("cat " +
                                     path +
                                     file_name +
                                     "/ibnetdiscover_p_" +
                                     suffix +
                                     " | grep -v -i sfb | grep -e ^SW | grep " +
                                     GUIDS[index] +
                                     "..x")
        ...
        if (st == 0):
            ...
            st, guid_ports = get_status_output("cat " +
                                               path +
                                               file_name +
                                               "/ibnetdiscover_p_" +
                                               suffix +
            ...
            tmp = "(" + GUIDS[index] + ")"
            res += tmp.ljust(30, ' ')
            res += str(HCA_ports_count) + " HCA ports and " + \
                str(switch_ports_count) + " switch ports.\n"
    return res
...
def sm_master_is_handler(card, port):
    ...
    st, MasterLID = get_status_output(
        "/usr/sbin/sminfo -C " + card + " -P " + port + " | awk '{print $4}'")
    if (st != 0):
        return "Could not retrieve Master LID. Reason: Could not run " + '"' + \
            "/usr/sbin/sminfo -C " + card + " -P " + port + " | awk '{print $4}'" + '"'
    st, all_sms = get_status_output("/usr/sbin/smpquery nodedesc " + MasterLID)
    if (st != 0):
        return "Could not retrieve all SM. Reason: Could not run " + \
            '"' + "/usr/sbin/smpquery nodedesc " + MasterLID + '"'
    res = "IB fabric SM master is: (" + all_sms + ")\nAll SMs in the fabric: "

    st, SMS = get_status_output("saquery -s -C " + card + " -P " + port +
                                " 2>/dev/null |grep base_lid |head -1| sed 's/\\./ /g'|awk '{print $2}'")
    if (st != 0):
        return "Could not retrieve all SM. Reason: Could not run " + '"' + "saquery -s -C " + card + \
            " -P " + port + " 2>/dev/null |grep base_lid |head -1| sed 's/\\./ /g'|awk '{print $2}'" + '"'
    SMS = set(SMS.split())

    for SM in SMS:
        st, smquery_nodedesc = get_status_output(
            "/usr/sbin/smpquery nodedesc " + SM)
        if (st != 0):
            smquery_nodedesc = "Could not run " + '"' + \
                "/usr/sbin/smpquery nodedesc " + SM + '"'
        st, sminfo = get_status_output(
            "/usr/sbin/sminfo -C " + card + " -P " + port + " " + SM)
        if (st != 0):
            sminfo = "Could not run " + '"' + "/usr/sbin/sminfo -C " + \
                card + " -P " + port + " " + SM + '"'
        res += "\n\nSM: " + SM + "\n" + smquery_nodedesc + "\n" + sminfo
...
def sm_status_handler(card, port):
    ...
    for lo in range(0, 4):
        ...
        st, SmActivity = get_status_output(
            "sminfo -C " + card + " -P " + port + " |awk '{ print $10 }'")
        if (st != 0):
            SmActivity = "<N/A>"
        ...
        if (SmActivity != "<N/A>" and represents_Int(SmActivity, 10)):
...
def sm_version_handler():
    if (cur_os != "debian"):
        st, res = get_status_output(
            "echo OpenSM installed packages: ; rpm -qa | grep opensm")
        if (st != 0):
            res = "Couldn't find command: echo OpenSM installed packages: ; rpm -qa | grep opensm"
    else:
        st, res = get_status_output(
            "echo OpenSM installed packages: ; dpkg -l | grep opensm")
        if (st != 0):
...
def ibdiagnet_handler(card, port, ibdiagnet_suffix):
    ...
    if (ibdiagnet_is_invoked == False):
        if (os.path.exists(path + file_name + "/" +
                           ibdiagnet_suffix + "/ibdiagnet") == False):
            os.mkdir(path + file_name + "/" + ibdiagnet_suffix + "/ibdiagnet")
        st, ibdiagnet_res = get_status_output(ibdiagnet_command +
                                              card +
                                              " -p " +
                                              port +
                                              " -o " +
...
def clean_ibnodes(ibnodes, start_string):
    ...
    for ibnode in ibnodes:
        if (ibnode.lower().startswith(start_string)):
            res += ibnode + "\n"
    return res
        ...
        for port_obj in active_subnets[card]:
            ...
            if (command == "ib_find_bad_ports"):
                ...
                result += ib_switches_FW_scan_handler(
                    port_obj["ibswitches"]["ibswitches_st"],
                    port_obj["ibswitches"]["ibswitches_output"],
                    ibdiagnet_suffix)
                ...
                # invoking regular command
                status, command_result = get_status_output(
                    command + " -C " + card + " -P " + port)
                if (status != 0):
...
def add_ext_file_handler(field_name, out_file_name, command_output):
    external_command = [
        ...
        "interrupts"]
    forbidden_chars = re.compile('([\\/:*?"<||-])')
    out_file_name = forbidden_chars.sub(
        r'', out_file_name).replace(
        "\\", "")  # clean the file name
    ...
    if (out_file_name != "pkglist" and ("erformance" not in out_file_name)
            and ("sr_iov" not in out_file_name)):
        f = open(full_Path, 'a+')
        if sys.version_info[0] == 2:
    ...
    if not ("mlnx_tune" in field_name) and not (
            "pcie_debug" in out_file_name or "pcie_debug_dict" in out_file_name):
        external_files_dict[field_name] = "<td><a href=" + \
            file_path + ">" + field_name + "</a></td>"
    ...
    if (field_name.startswith("var/log")):
        if is_command_allowed("file: " + curr_path):
            ...
            if (status == 0 and command_output):
                ...
                add_ext_file_handler(
                    field_name, filtered_field_name, command_output)
            else:
                err_flag = 1
        ...
        if is_command_allowed("file :" + curr_path):
            ...
            if (status == 0):
                ...
                if (st == 0):
                    add_ext_file_handler(
                        field_name, "config-" + uname.strip(), command_output)
                else:
                    err_flag = 1
                    err_command += "uname -r"
            else:
        ...
        if is_command_allowed("mlxcables --DDM/--dump"):
            ...
            if st != 0:
                ...
                external_files_dict[field_name] = "<td><a href=" + \
                    curr_path + ">" + field_name + "</a></td>"
                available_external_files_collection.append(
                    [field_name, curr_path])
    elif (field_name == "config.gz"):
        if is_command_allowed("file : /proc/config.gz"):
            if (os.path.isfile('/proc/config.gz')):
                status, command_output = get_status_output(
                    "cp /proc/config.gz " + path + file_name)
                if (status != 0):
                    err_flag = 1
        ...
        if is_command_allowed("ps -eLo"):
            status, command_output = get_status_output(
                "ps -eLo lstart,%cpu,psr,nlwp,f,uid,pid,ppid,pri,rtprio,ni,vsz,rss,stat,tty,time,wchan,args")
            if (status == 0):
                add_ext_file_handler(field_name, field_name, command_output)
            else:
                err_flag = 1
        ...
        if is_command_allowed("ibnetdiscover", "no_ib"):
            if (is_ib == 0 and no_ib_flag == False):
                ...
                status, command_output = get_status_output(
                    ibnetdiscover_command)
                if (status == 0):
                    add_ext_file_handler(
                        "ibnetdiscover", "ibnetdiscover", command_output)
                    for card in active_subnets:
                        for port in active_subnets[card]:
                            status, command_output = get_status_output(
                                ibnetdiscover_command + " -p -C " + card + " -P " + port["port_num"])
                            suffix = card + "_" + port["port_num"]
                            if (status == 0):
                                add_ext_file_handler(
                                    "ibnetdiscover --virt -p " + suffix,
                                    "ibnetdiscover_p_" + suffix,
                                    command_output)
                            else:
                                err_flag = 1
        ...
        if is_command_allowed("Installed_packages"):
            if (cur_os != "debian"):
                status, unrelevant_res = get_status_output(
                    "rpm -qa --last > " + path + file_name + "/pkglist")
            else:
                status, unrelevant_res = get_status_output(
                    "dpkg --list > " + path + file_name + "/pkglist")
            if (status == 0):
        ...
        if is_command_allowed(
            "get_ib_sys_files_exclude_uevent_files",
                "no_ib"):
            error_files = ""
            if (no_ib_flag == False):
                st, res = get_status_output(
                    "find /sys | grep infini |grep -v uevent |sort")
                if (st == 0 and res != ""):
        ...
        if is_command_allowed("get_numa_node_sys_files_exclude_uevent_files"):
            ...
            st, numas = get_status_output(
                "find /sys | grep numa_node | grep -v uevent |sort")
            if st == 0:
                command_output = ""
        ...
        if is_command_allowed("mlnx_tune", "no_ib"):
            status, command_output = get_status_output(
                "./mlnx_tune -r -i " + path + file_name, "1m")
            if not (("No such file or directory" in command_output) or (
                    (status != 0) and not ("Unsupported" in command_output))):
...
def arrange_internal_files_section():
    if (cur_os == "debian"):
        internal_files_collection.extend(
            ["/etc/debian_version", "/etc/network/interfaces"])
    if verbose_flag:
    ...
    if is_command_allowed("file: /etc/modprobe.d/"):
        if (os.path.exists("/etc/modprobe.d/")):
            for file in os.listdir("/etc/modprobe.d/"):
                if (os.path.isfile("/etc/modprobe.d/" + file)):
                    if verbose_count == 2:
    ...
    if is_command_allowed("file: /etc/netplan/"):
        if (os.path.exists("/etc/netplan/")):
            for file in os.listdir("/etc/netplan/"):
                if (os.path.isfile("/etc/netplan/" + file)):
                    if verbose_count == 2:
    ...
    if is_command_allowed("file: /proc/net/vlan/"):
        if (os.path.exists("/proc/net/vlan/")):
            for file in os.listdir("/proc/net/vlan/"):
                if (os.path.isfile("/proc/net/vlan/" + file)):
                    if verbose_count == 2:
                ...
                if (os.path.isfile("/sys/devices/system/node/" + file) == False):
                    if verbose_count == 2:
                        print(
                            ...
                            file +
                            "/cpulist - start")
                    add_internal_file_if_exists(
                        "/sys/devices/system/node/" + file + "/cpulist")
                    if verbose_count == 2:
                        print(
                            "\t\t/sys/devices/system/node/" +
                            file +
    ...
    if is_command_allowed("file: /etc/sysconfig/network-scripts/ifcfg*"):
        if (cur_os != "debian" and os.path.exists(
                "/etc/sysconfig/network-scripts/")):
            for file in os.listdir("/etc/sysconfig/network-scripts/"):
                if ((os.path.isfile("/etc/sysconfig/network-scripts/" + file))
                        and (file.startswith("ifcfg"))):
                    if verbose_count == 2:
                        print(
                            "\t\t/etc/sysconfig/network-scripts/" +
                            file +
    ...
    if is_command_allowed("file: /etc/sysconfig/network/ifcfg*"):
        if (os.path.exists("/etc/sysconfig/network/")):
            for file in os.listdir("/etc/sysconfig/network/"):
                if ((os.path.isfile("/etc/sysconfig/network/" + file))
                        and (file.startswith("ifcfg-"))):
                    if verbose_count == 2:
                        print(
    ...
    if is_command_allowed("file: /etc/*release*"):
        if (os.path.exists("/etc/")):
            for file in os.listdir("/etc/"):
                if ((os.path.isfile("/etc/" + file)) and ("release" in file)):
                    if verbose_count == 2:
    ...
    if is_command_allowed("file: /etc/infiniband/"):
        if (os.path.exists("/etc/infiniband/")):
            for file in os.listdir("/etc/infiniband/"):
                if (os.path.isfile("/etc/infiniband/" + file)
                    ) and (file != "vf-net-link-name.sh"):
                    if verbose_count == 2:
...
def arrange_external_files_section():
    ...
    if st == 0:
        ...
        for file in var_log_files:
            if file == 'syslog' or file == 'messages' or file == 'boot.log':
                add_external_file_if_exists(
                    'var/log/' + file, 'var/log/' + file)
    # add external files if exist to the provided external section e.g.
    # "kernel config"
    ...
    if status == 0:
        ...
        if print_err_flag == 1:
            f = open(
                path +
                file_name +
                "/err_messages/dummy_sriov_functions",
                'a')
            f.write("The full command is: " + command + "\n")
            f.write(result)
            f.write("\n\n")
    ...
    if os.path.exists("/sys/class/infiniband/"):
        for indir in os.listdir("/sys/class/infiniband/"):
            if is_command_allowed("file: /sys/class/infiniband/*/device/"):
                if os.path.exists(
                        "/sys/class/infiniband/" + indir + "/device/"):
                    for infile in os.listdir(
                            "/sys/class/infiniband/" + indir + "/device/"):
                        if (infile.startswith("sriov") or infile.startswith("mlx")) and os.path.isfile(
                                "/sys/class/infiniband/" + indir + "/device/" + infile):
                            if verbose_count == 2:
                                print(
                                    "\t\t\t/sys/class/infiniband/" +
                                    indir +
                                    ...
                                    infile +
                                    " - start")
                            add_sriov_internal_file_if_exists(
                                "/sys/class/infiniband/" + indir + "/device/" + infile)
                            if verbose_count == 2:
                                print("\t\t\t/sys/class/infiniband/" +
                                      indir + "/device/" + infile + " - end")
            if os.path.exists("/sys/class/infiniband/" + indir + "/iov/"):
                if is_command_allowed("file: /sys/class/infiniband/*/iov"):
                    if os.path.exists(
                            "/sys/class/infiniband/" + indir + "/iov/ports/"):
                        for indir2 in os.listdir(
                                "/sys/class/infiniband/" + indir + "/iov/ports/"):
                            if represents_int(indir2) and int(
                                    indir2) >= 0 and int(indir2) <= 127:
                                if os.path.isfile(
                                    "/sys/class/infiniband/" +
                                    indir +
                                    "/iov/ports/" +
                                    ...
                                    if verbose_count == 2:
                                        print(
                                            ...
                                            " - start")
                                    add_sriov_internal_file_if_exists(
                                        "/sys/class/infiniband/" + indir + "/iov/ports/" + indir2 + "/gids/" + indir2)
                                    if verbose_count == 2:
                                        print(
                                            "\t\t\t/sys/class/infiniband/" +
                                            indir +
                                    ...
                                    if os.path.isfile(
                                        ...
                                        if verbose_count == 2:
                                            print(
                                                ...
                                                " - start")
                                        add_sriov_internal_file_if_exists(
                                            "/sys/class/infiniband/" + indir + "/iov/ports/" + indir2 + "/pkeys/" + indir2)
                                        if verbose_count == 2:
                                            print(
                                                "\t\t\t/sys/class/infiniband/" +
                                                indir +
                                                ...
                                                " - end")
                    for indir2 in os.listdir(
                            "/sys/class/infiniband/" + indir + "/iov/"):
                ...
                if indir.endswith("core"):
                    if verbose_count == 2:
                        print(
                            ...
                            "/unbind - start")
                    add_sriov_internal_file_if_exists(
                        "/sys/bus/pci/drivers/" + indir + "/unbind")
                    if verbose_count == 2:
                        print(
                            "\t\t\t/sys/bus/pci/drivers/" +
                            indir +
                    ...
                    if verbose_count == 2:
                        print(
                            ...
                            "/bind - start")
                    add_sriov_internal_file_if_exists(
                        "/sys/bus/pci/drivers/" + indir + "/bind")
                    if verbose_count == 2:
                        print(
                            "\t\t\t/sys/bus/pci/drivers/" +
                            indir +
                ...
                if infile.startswith("ifcfg-"):
                    if verbose_count == 2:
                        print(
                            ...
                            " - start")
                    add_sriov_internal_file_if_exists(
                        "/etc/sysconfig/network-scripts/" + infile)
                    if verbose_count == 2:
                        print(
                            "\t\t\t/etc/sysconfig/network-scripts/" +
                            infile +
        ...
        if os.path.exists("/sys/class/net/"):
            for indir in os.listdir("/sys/class/net/"):
                if os.path.isfile(
                        "/sys/class/net/" +
                        indir) == False and (
                        indir.startswith("eth") or indir.startswith("ib")):
                    for inSomething in os.listdir(
                            "/sys/class/net/" + indir + "/"):
                        if os.path.isfile(
                                "/sys/class/net/" + indir + "/" + inSomething) == False:
                            if os.path.isfile(
                                    "/sys/class/net/" + indir + "/" + inSomething + "/tx_rate"):
                                if verbose_count == 2:
                                    print(
                                        "\t\t\t/sys/class/net/" +
                                        indir +
                            ...
                            if os.path.isfile(
                                    "/sys/class/net/" + indir + "/" + inSomething):
                                if verbose_count == 2:
                                    print(
                                        "\t\t\t/sys/class/net/" +
                                        indir +
                                ...
                                if verbose_count == 2:
                                    print(
                                        "\t\t\t/sys/class/net/" + indir + "/" + inSomething + " - end")
        ...
        if os.path.exists("/sys/bus/pci/devices/"):
            for indir in os.listdir("/sys/bus/pci/devices/"):
                if os.path.isfile("/sys/bus/pci/devices/" + indir) == False:
                    if os.path.isfile(
                            "/sys/bus/pci/devices/" + indir + "/reset"):
                        if verbose_count == 2:
                            print(
                                "\t\t\t/sys/bus/pci/devices/" +
                                indir +
...
def print_destination_out_file():
    ...
    if (os.path.exists(path + file_name)
            and os.path.isfile(path + file_name) == False):
        ...
        for fi in sorted(os.listdir(path + file_name)):
            if (fi == "err_messages"):
                ...
                print(
                    "ibdiagnet \t\t\t- contains all files generated from invoking ibdiagnet")
                ...
                print(
                    "conf_nova \t\t- contains all files under /var/lib/config-data/puppet-generated/nova_libvirt")
            elif (fi == "conf_neutron"):
                print(
                    "conf_neutron \t\t- contains all files under /var/lib/config-data/puppet-generated/neutron/")
            elif (fi == "conf_nove"):
                print(
                    "logs_nova \t\t- contains all files under /var/log/containers/nova")
            elif (fi == "conf_nove"):
                print(
                    "logs_neutron \t\t- contains all files under /var/log/containers/neutron")
            else:
                print(fi)
...
def remove_unwanted_files():
    ...
    if (path != "/tmp/" and os.path.exists(path)):
...
def validate_path():
    ...
    for ch in taboo_chars:
        if ch in path:
            print_invalid_path_and_exit(
                "Invalid Path: directory name should not contain " + ch)
    ...
    for name in names:
        if (name.startswith("-") or name.startswith("_")):
            print_invalid_path_and_exit(
                ...
                name +
                " should not start with of any of a space,hyphen or underscore")
...
def hyper_threading():
    key = "Hyper Threading"
    st, siblings = get_status_output(
        "cat /proc/cpuinfo | grep " + '"' + "siblings" + '"')
    if (st != 0):
        st, siblings_lines_num = get_status_output(
            "cat /proc/cpuinfo | grep " + '"' + "siblings" + '"' + " | wc -l")
        if (st == 0 and represents_int(siblings_lines_num)
                and int(siblings_lines_num) == 0):
            siblings = ""
        else:
            perf_val_dict[key] = "command not found: cat /proc/cpuinfo | grep " + \
                '"' + "siblings" + '"'
            return

    st, cpu_cores = get_status_output(
        "cat /proc/cpuinfo | grep " + '"' + "cpu cores" + '"')
    if (st != 0):
        st, cpu_cores_lines_num = get_status_output(
            "cat /proc/cpuinfo | grep " + '"' + "cpu cores" + '"' + " | wc -l")
        if (st == 0 and represents_int(cpu_cores_lines_num)
                and int(cpu_cores_lines_num) == 0):
            cpu_cores = ""
        else:
            perf_val_dict[key] = "command not found: cat /proc/cpuinfo | grep " + \
                '"' + "cpu cores" + '"'
...
def ip_forwarding():
    ...
    if (ip_forwarding_status["IPv4"] ==
            not_present or ip_forwarding_status["IPv6"] == not_present):
        perf_status_dict[key] = "Warning"
    else:
        perf_status_dict[key] = "OK"
...
def performance_lspci(check_latest=False):
    ...
    # lspci -d 15b3: - e.g 81:00.0 Ethernet controller: Mellanox Technologies
    # MT27800 Family [ConnectX-5]
    st, cards_num = get_status_output("lspci -d 15b3: | wc -l")
    if (st != 0 or ("command not found" in cards_num)
            or represents_int(cards_num) == False):
        perf_status_dict[key] = "OK"
    ...
    for card in mlnx_cards:
        ...
        pci_devices.append({"status": "OK",
                            ...
                            "desired_width": 8.0,
                            "current_width": 8.0,
                            "desired_payload_size": 256.0,
                            "current_payload_size": 8.0,
                            "desired_max_read_request": 4096.0,
                            "current_max_read_request": 4096.0})
        ...
        if (("-ib" in card) or ("pro" in card) or ("x-3" in card) or ("x3" in card)
                or ("x-4" in card) or ("x4" in card) or ("connectib" in card)):
            pci_devices[i]["desired_gen"] = 3.0
        else:
            if ("x-5" in card) or ("x5" in card) or ("x6" in card) or (
                    "x-6" in card) or ("MT27630" in card_str) or ("MT28908" in card_str):
                ...
                pci_devices[i]["desired_payload_size"] = 256.0
                pci_devices[i]["desired_max_read_request"] = 512.0
        st, firmwares_query, tool_used = general_fw_command_output(
            'fwflint_q', card_pci)
        if (st == 0):
            # firmwares_query :-
            ...
            if check_latest:
                if is_command_allowed(
                    'mlxfwmanager --online-query-psid',
                        "check_fw"):
                    st, check_latest_fw = get_status_output(
                        "mlxfwmanager --online-query-psid " + pci_devices[i]["psid"], "30s")
                    if (st == 0):
                        #     FW             14.20.1010
                        check_latest_fw = re.search(
                            "FW((.*\n))", check_latest_fw, re.MULTILINE).group(0).splitlines()
                        if LooseVersion(check_latest_fw[0].split(
                        )[-1]) > LooseVersion(pci_devices[i]["current_fw"]):
                            pci_devices[i]["status"] = "Warning"
                            pci_devices[i]["current_fw"] = 'Warning the current Firmware- ' + \
                                pci_devices[i]["current_fw"] + ', is not latest - ' + check_latest_fw[0].split()[-1]
                        else:
                            pci_devices[i]["current_fw"] = pci_devices[i]["current_fw"] + \
                                "(latest version)"
                else:
                    running_warnings.append(
                        "--check_fw flag was provided but mlxfwmanager --online-query-psid command not allowed")
    if is_all_failed:
        running_warnings.append(
            "--check_fw flag was provided but running flint q for all cards failed")
    ...
    for card_xxx in cards_xxx:
        ...
        if (len(card_cur_gen) > 1):
            ...
            pci_devices[i]["current_gen"] = -1.0
    st, cards_speed_width = get_status_output(
        "lspci -d 15b3: -vvv | grep -i Speed")
    st, cards_speed_width = get_status_output(
        "lspci -d 15b3: -vvv | grep -i Speed")
    if (st != 0):
    ...
    for line in cards_speed_width:
        ...
        if ("lnksta:" in line):
            i += 1
            try:
                pci_devices[i]["current_speed"] = float(
                    (line.split("gt/s")[0]).split()[-1])
            except ValueError:
                pci_devices[i]["current_speed"] = -1.0
            if (len(line.split("width x")) > 1):
                try:
                    pci_devices[i]["current_width"] = float(
                        re.split(',|[(]| ', line.split("width x")[1])[0])
                except ValueError:
            ...
            try:
                pci_devices[i]["desired_speed"] = float(
                    (line.split("gt/s")[0]).split()[-1])
            except ValueError:
                pci_devices[i]["desired_speed"] = -1.0
            if (len(line.split("width x")) > 1):
                try:
                    pci_devices[i]["desired_width"] = float(
                        re.split(',|[(]| ', line.split("width x")[1])[0])
                except ValueError:
    ...
    st, cards_payload_read = get_status_output(
        "lspci -d 15b3: -vvv | grep -i MaxReadReq")
    if (st != 0):
    ...
    for line in cards_payload_read:
        ...
        if "maxreadreq" in line:
            i += 1
            try:
                pci_devices[i]["current_payload_size"] = float(
                    (line.split(" bytes,")[0]).split()[-1])
            except ValueError:
                pci_devices[i]["current_payload_size"] = -1.0
            if (len(line.split("maxreadreq ")) > 1):
                try:
                    pci_devices[i]["current_max_read_request"] = float(
                        (line.split("maxreadreq ")[1]).split()[0])
                except ValueError:
    ...
    for i in range(0, len(pci_devices)):
        ...
        PCIE_debug_info = {
            "name": pci_devices[i]["name"],
            "current_fw": pci_devices[i]["current_fw"],
            "psid": pci_devices[i]["psid"]}
        PCIE_debugging_information.append(PCIE_debug_info)
    add_output_to_pcie_debug_dict(
        "devices_information",
        PCIE_debugging_information)
...
def memlock():
    ...
    st, count = get_status_output(
        "cat /etc/security/limits.conf | grep memlock | wc -l")
    if (st != 0 or ("command not found" in count)
            or (count == "") or (count == "0")):
        perf_status_dict[key] = not_present
        perf_val_dict[key] = ""
    else:
        st, memlock = get_status_output(
...
def bw_and_lat():
    ...
    if is_command_allowed('perf_samples', "no_ib,perf"):
        for pf_device in pf_devices:
            ...
            st, pfc_output = get_status_output(
                "ethtool -S " + pf_device + " | grep -e pause -e discards", "20s")
            if st == 0:
                perf_samples[pf_device] += "Before test sample: ethtool -S " + \
                    pf_device + " | grep -e pause -e discards \n" + pfc_output + "\n"
            st, pfc_output = get_status_output(
                "ethtool -S " + pf_device + " | grep -e prio", "20s")
            if st == 0:
                perf_samples[pf_device] += "Before test sample: ethtool -S " + \
                    pf_device + " | grep -e prio \n" + pfc_output + "\n"
        ## ------------------------------------End samples before test------------------------------------------------##
        for device in devices:
            perf_samples[device] = ""
            st, cnp_files = get_status_output(
                "ls /sys/class/infiniband/" + device + "/ports/1/hw_counters/*cnp*")
            cnp_files = cnp_files.split()
        ## ------------------------------------Samples before test------------------------------------------------##
            for cnp_file in cnp_files:
                st, cnp_output = get_status_output("cat " + cnp_file)
                if st == 0:
                    perf_samples[device] += "Before test sample: cat " + \
                        cnp_file + "\n" + cnp_output + "\n"
            st, ecn_output = get_status_output(
                "cat /sys/class/infiniband/" + device + "/ports/1/hw_counters/np_ecn_marked_roce_packets")
            if st == 0:
                perf_samples[device] += "Before test sample: cat /sys/class/infiniband/" + \
                    device + "/ports/1/hw_counters/np_ecn_marked_roce_packets \n" + ecn_output + "\n"
            perf_samples[device] += "\n\n------------------------------------------------\n\n"
        ## ------------------------------------End samples before test------------------------------------------------##
        if is_command_allowed('ib_write_bw_test', "no_ib,perf"):
            bandwidth[device] = ""
            cmd = "ib_write_bw --report_gbits -d " + device + \
                " >/dev/null & sleep 2; ib_write_bw --report_gbits -d " + device + " localhost"
            st, bandwidth[device] = getstatusoutput(cmd)
            bandwidth[device] = cmd + "\n\n" + bandwidth[device]
        if is_command_allowed('latency', "no_ib,perf"):
            cmd = "ib_write_lat -d " + device + \
                " >/dev/null & sleep 2; ib_write_lat -d " + device + " localhost"
            st, latency[device] = getstatusoutput(cmd)
            latency[device] = cmd + "\n\n" + latency[device]
        if device in show_gids:
            try:
                data = show_gids.split(
                    device)[-1].splitlines()[0].strip().split()
                if len(data) >= 5:
                    index = data[1]
                    if is_command_allowed('ib_write_bw_test', "no_ib,perf"):
                        bandwidth[device] += "\n\n##################################################\n\n"
                        cmd = " ib_write_bw -d " + device + " -x" + index + \
                            " >/dev/null & sleep 2; ib_write_bw -d " + device + " -x" + index + " localhost"
                        bandwidth[device] += cmd + "\n\n"
                        st, bandwidth_x = getstatusoutput(cmd)
                        bandwidth[device] += bandwidth_x
                    if is_command_allowed('latency', "no_ib,perf"):
                        latency[device] += "\n\n##################################################\n\n"
                        cmd = " ib_write_lat -d " + device + " -x" + index + \
                            " >/dev/null & sleep 2; ib_write_lat -d " + device + " -x" + index + " localhost"
                        latency[device] += cmd + "\n\n"
        ...
        if is_command_allowed('perf_samples', "no_ib,perf"):
            for cnp_file in cnp_files:
                st, cnp_output = get_status_output("cat " + cnp_file)
                if st == 0:
                    perf_samples[device] += "After test sample: cat " + \
                        cnp_file + "\n" + cnp_output + "\n"
            st, ecn_output = get_status_output(
                "cat /sys/class/infiniband/" + device + "/ports/1/hw_counters/np_ecn_marked_roce_packets")
            if st == 0:
                perf_samples[device] += "After test sample: cat /sys/class/infiniband/" + \
                    device + "/ports/1/hw_counters/np_ecn_marked_roce_packets \n" + ecn_output + "\n"
    ...
    if is_command_allowed('perf_samples', "no_ib,perf"):
        for pf_device in pf_devices:
            st, pfc_output = get_status_output(
                "ethtool -S " + pf_device + " | grep -e pause -e discards", "20s")
            if st == 0:
                perf_samples[pf_device] += "After test sample: ethtool -S " + \
                    pf_device + " | grep -e pause -e discards \n" + pfc_output + "\n"
            st, pfc_output = get_status_output(
                "ethtool -S " + pf_device + " | grep -e prio", "20s")
            if st == 0:
...
def perform_checkings():
    ...
    elif (no_ib_flag == False and perf_flag):
        if verbose_count == 2:
...
def initialize_html(html_flag):
    ...
    html.write("<body><pre>")
    html.write("<a name=" + '"' + "index" + '"' +
               "></a><h1>Mellanox Technologies</h1>")
    html.write("<br/>")
    html.write(
        "<a name=" +
        '"' +
    ...
    if (non_root):
        html.write(
            ...
            "3" +
            '"' +
            ">Warning: Running as non root user, commands/files that require root permissions are missing. (--non_root flag was provided)</font></p>")
    ...
    if no_fw_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: Firmware commands are NOT included. (--no_fw flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: Firmware commands are included.</font></p>")
    ...
    if mtusb_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: I2C firmware commands are included. (--mtusb flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: I2C firmware commands are NOT included. (--mtusb flag was not provided)</font></p>")
    ...
    if no_ib_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: IB commands are NOT included, hence e.g. NO fabric commands section (--no_ib flag was provided)</font></p>")
        if ibdiagnet_flag:
            html.write(
                "<p><font color=" +
                '"' +
                ...
                "3" +
                '"' +
                ">Alert: --ibdiagnet flag was provided, it is ineffective since --no_ib flag was provided</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: IB commands are included. (--no_ib flag was not provided)</font></p>")
        if ibdiagnet_flag:
            if is_ib != 0:
                html.write(
                    "<p><font color=" +
                    ...
                    "3" +
                    '"' +
                    ">Error: --ibdiagnet flag was provided, but could NOT generate ibdiagnet</font></p>")
                ...
                html.write(
                    ...
                    "3" +
                    '"' +
                    ">Error: --ibdiagnet flag was provided, but it's ouput may not be displayed correctly for all subnets</font></p>")
            else:
                html.write(
                    "<p><font color=" +
                    '"' +
                    ...
                    "3" +
                    '"' +
                    ">Alert: ibdiagnet command is included. (--ibdiagnet flag was provided)</font></p>")
        else:
            html.write(
                "<p><font color=" +
                '"' +
                ...
                "3" +
                '"' +
                ">Alert: ibdiagnet command is NOT included. (--ibdiagnet flag was not provided)</font></p>")
    ...
    if with_inband_flag and is_MFT_installed:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: In-band cables information are included (--with_inband flag was provided)</font></p>")
        ...
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: In-band cables information are NOT included (--with_inband flag was not provided)</font></p>")
        ...
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: In-band cables information are NOT included (--with_inband flag was provided, but could NOT generate in-band cables information)</font></p>")
    if openstack_flag:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: OpenStack commands are included (--openstack flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: OpenStack commands are NOT included. (--openstack flag was not provided)</font></p>")
    ...
    if asap_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: Asap commands are included (--asap flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: Asap commands are NOT included. (--asap flag was not provided)</font></p>")
    ...
    if fsdump_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: fsdump command is included (--fsdump flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: fsdump commands is NOT included. (--fsdump flag was not provided)</font></p>")
    ...
    if asap_tc_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: Asap TC filter commands are included (--asap_tc flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: Asap TC filter commands are NOT included. (--asap_tc flag was not provided)</font></p>")
    ...
    if rdma_debug_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: Rdma commands with iprout2 are included. (--rdma debug flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: Rdma commands with iprout2 are NOT included. (--rdma debug flag was not provided)</font></p>")
    if gpu_flag:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: Nvidia GPU commands are included. (--gpu flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: Nvidia GPU commands are NOT included. (--gpu flag was not provided)</font></p>")
    ...
    if (mlnx_cards_status < 0):
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: Unable to count Mellanox cards. </font></p>")
        ...
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: There are no Mellanox cards. </font></p>")
    ...
    if pcie_flag:
        html.write(
            ...
            "3" +
            '"' +
            ">Alert: pcie commands are included. (--pcie flag was provided)</font></p>")
    else:
        html.write(
            "<p><font color=" +
            '"' +
            ...
            "3" +
            '"' +
            ">Alert: pcie commands are NOT included. (--pcie flag was not provided)</font></p>")
    html.close()
...
def html_write_section(html, title, collection, base):
    ...
    if (collection == available_commands_collection):
        collection = sorted(chain.from_iterable(collection))
    rows = len(collection) // 4
    mod_val = len(collection) % 4

    c = 0
    r = 0
    html.write("<!-- rows: " + str(rows) + " " +
               title + str(len(collection)) + " -->")

    for i in range(len(collection)):
        if (c <= mod_val):
            cmd = r + c * (rows + 1)
        else:
            cmd = r + mod_val * (rows + 1) + (c - mod_val) * \
                rows  # = r + mod_val + c*rows
...
def html_write_prev(html, sec):
    ...
    if (sec != 1001):
        html.write("<small><a href=" + '"' + "#sec" +
                   str(sec - 1) + '"' + ">[&lt;&lt;prev]</a></small> ")


def html_write_index(html):
    # Add index button
    html.write("<small><a href=" + '"' + "#index" +
               '"' + ">[back to index]</a></small> ")


def html_write_next(html, sec):
    # Add next button if not last line
    html.write("<small><a href=" + '"' + "#sec" +
               str(sec + 1) + '"' + ">[next>>]</a></small> ")
...
def html_write_paragraph(html, base, collection, dict, prev_parag_end):
    ...
    for i in range(len(collection)):
        ...
        if ((i + 1) == len(collection)):
            ...
            if (base < 3000):
                if (is_ib == 0 and no_ib_flag == False):  # IB Fabric
                    html.write("<small><a href=" + '"' + "#sec" +
                               str(base + 1000 + 1) + '"' + ">[next>>]</a></small> ")
                else:
                    html.write("<small><a href=" + '"' + "#sec" +
                               str(base + 2000 + 1) + '"' + ">[next>>]</a></small> ")
        ...
        if ((original_collection == available_commands_collection) and (
                collection[i] in available_commands_collection[not is_command_string])):
            array_output_links_collection = [
                "fw_ini_dump",
                "mst_commands_query_output",
                "asap_parameters",
                "asap_tc_information",
                ...
                "networkManager_system_connections"]
            html.write("<p>")
            if (collection[i] == "ethtool_all_interfaces") or (
                    collection[i] == "devlink_handler"):
                content = dict[collection[i]]
                content = content.split("\n")
                content_final = ""
                for line in content:
                    if "<td><a href=" not in line:
                        content_final += line.replace('<',
                                                      "&lt;").replace('>', "&gt;") + "\n"
                    else:
                        content_final += line + "\n"
                ...
                if ("mst_commands_query_output" in collection[i] and non_root):
                    html.write(
                        "Running as a non-root user - You must be root to use mst tool")
                    html.write("&nbsp;&nbsp;&nbsp;&nbsp;")
                else:
...
def build_and_finalize_html():
    ...
    if (st_saquery == 0 and no_ib_flag == False):
        html_write_section(
            html,
            "2. Fabric Diagnostic Information: ",
            available_fabric_commands_collection,
            2000)
    ...
    html.write("<!-- rows: " + str(rows) + " External Files: " +
               str(len(available_external_files_collection)) + " -->")
    ...
    parag_1_end = html_write_paragraph(
        html,
        1000,
        available_commands_collection,
        server_commands_dict,
    ...
    if (st_saquery == 0 and no_ib_flag == False):  # IB Fabric
        parag_2_end = html_write_paragraph(
            html,
            2000,
            available_fabric_commands_collection,
            fabric_commands_dict,
    ...
    parag_3_end = html_write_paragraph(
        html,
        3000,
        available_internal_files_collection,
        files_dict,
...
def initialize_html2(html2_flag):
    ...
    html2.write("<a name=" + '"' + "index" + '"' +
                "></a><h1>Mellanox Technologies</h1>")
    html2.write("<br/>")
    html2.write("<a name=" + '"' + "index" + '"' +
                "></a><h2>Performance Tuning Analyze for Linux</h2>")
    html2.write("<br/>")
    html2.write(
        "<a name=" +
        '"' +
...
def html2_write_ipv(html2, key):
    ...
    if (ip_forwarding_status[key] == not_present):
        ...
        html2.write(
            ...
            '"' +
            ">" +
            ip_forwarding_status[key] +
            "</font>")
    html2.write(
        "<br/>" +
        key +
        " Forwarding value: " +
        ip_forwarding_val[key] +
        "<br/>")
...
def html2_write_lspci(html2):
    ...
    for i in range(len(pci_devices)):
        ...
        if (pci_devices[i]["desired_gen"] != not_available):
            html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Desired PCIe Generation: " +
                        str(int(pci_devices[i]["desired_gen"])))
            html2.write("<br/>")

        # print current gen
        if (pci_devices[i]["current_gen"] != not_available):
            html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Current PCIe Generation: " +
                        str(int(pci_devices[i]["current_gen"])))
            html2.write("<br/>")

        # print desired speed
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Desired Speed: " +
                    str(pci_devices[i]["desired_speed"]))
        html2.write("<br/>")

        # print current speed
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Current Speed: " +
                    str(pci_devices[i]["current_speed"]))
        html2.write("<br/>")

        # print desired width
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Desired Width: x" +
                    str(pci_devices[i]["desired_width"]))
        html2.write("<br/>")

        # print current width
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Current Width: x" +
                    str(pci_devices[i]["current_width"]))
        html2.write("<br/>")

        # print desired payload size
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Desired Payload Size: " +
                    str(pci_devices[i]["desired_payload_size"]))
        html2.write("<br/>")

        # print current payload size
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Current Payload Size: " +
                    str(pci_devices[i]["current_payload_size"]))
        html2.write("<br/>")

        # print desired max read request
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Desired Max Read Request: " +
                    str(pci_devices[i]["desired_max_read_request"]))
        html2.write("<br/>")

        # print current max read request
        html2.write("&nbsp;&nbsp;&nbsp;&nbsp;Current Max Read Request: " +
                    str(pci_devices[i]["current_max_read_request"]))
        html2.write("<br/>")
...
def html2_write_paragraph(html2, base, prev_parag_end):
    ...
    for i in range(len(perf_setting_collection)):
        ...
        if (perf_setting_collection[i] not in setting_without_status):
            if (perf_status_dict[perf_setting_collection[i]] == "Not OK"):
                html2.write("Status: <font color=" +
                            ...
                            '"' +
                            ">" +
                            perf_status_dict[perf_setting_collection[i]] +
                            "</font>")
        ...
        if (perf_setting_collection[i] == "Memory Allocation"):
            html2.write("Setting Value: <br/>" +
                        perf_val_dict[perf_setting_collection[i]].replace('<', "&lt;").replace('>', "&gt;"))
        ...
        elif ((perf_setting_collection[i] != "PCI Configurations") or (direct)):
            html2.write("Setting Value: " + perf_val_dict[perf_setting_collection[i]].replace(
                '<', "&lt;").replace('>', "&gt;"))
        else:
    ...
    html2.write("<small><a href=" + '"' + "#sec" + str(sec - 1) +
                '"' + ">[&lt;&lt;prev]</a></small> ")
    html_write_index(html2)
    html2.write("</p>")
...
def build_and_finalize_html2():
    ...
    if (no_ib_flag == False and os.path.exists(
            path + file_name + "/commands_txt_output/mlnx_tune_r")):
        ...
        html2.write("<!-- rows: " +
                    str(rows) +
                    " Perf External Files: " +
                    str(len(perf_external_files_collection)) +
                    " -->")
...
def initialize_html3(html3_flag):
    ...
    html3.write(
        ...
        "index" +
        '"' +
        "></a><h2>Single Root IO Virtualization (SR_IOV)</h2>")
    html3.write("<br/>")
    html3.write(
        "<a name=" +
        '"' +
...
def build_and_finalize_html3():
    ...
    if len(available_sriov_commands_collection) == 0 and len(
            available_sriov_internal_files_collection) == 0:
        html3.write(
            "There are neither available SR_IOV commands nor available SR_IOV related internal files")
        html3.write("</body></pre>")
        html3.write("</html>")
        html3.close()
        return
...
def confirm_mlnx_cards():
    ...
    if (st != 0 or ("command not found" in mlnx_cards)):
        ...
        mlnx_cards = mlnx_cards.splitlines()
        if len(mlnx_cards) > 0 and represents_Int(
                mlnx_cards[-1].split(":")[0], 16):
            mlnx_cards_status = 1
            for mlnx_card in mlnx_cards:
...
def load_modules():
    ...
    if mst_devices_exist:
        current_mst_devices = os.listdir("/dev/mst")
        for device in current_mst_devices:
            if ((device.startswith('SW') or device.startswith('CA'))
                    and ('cable') in device):
...
def generate_output():
    ...
    if (is_ib == 0 and no_ib_flag == False):
        ...
        for card in active_subnets:
            for port_obj in active_subnets[card]:
                ...
                ibdiagnet_suffix = "ibdiagnet_" + card + "_" + port
                if ibdiagnet_flag:
                    st, mkdir_output = get_status_output(
                        "mkdir " + path + file_name + "/" + ibdiagnet_suffix)
                    if st != 0:
                        ibdiagnet_error = True
    if verbose_flag:
    ...
    if config_file_flag:
        st, out = get_status_output(
            "cp " + config_path + " " + path + file_name)
    # Major operations for creating the .json file
    if (verbose_flag and json_flag and not generate_config_flag):
        print("\t----------------------------------------------------")
        print("\tGenerating JSON file has started")
    if (json_flag and json_found):
    ...
    elif (json_flag and not generate_config_flag):
        if verbose_flag:
    ...
    if generate_config_flag:
        ...
        for key, value in config_dict.items():
            csv_writer.writerow(
                [key, value["approved"], value["related flag"]])
        csvfile.close()
        print("\t----------------------------------------------------\n")
...
def update_flags(args):
        ...
        try:
            with open(config_path, 'r') as csvfile:
                ...
                config_dict = {
                    rows[0]: {
                        "approved": rows[1],
                        "related flag": rows[2]} for rows in reader}
    ...
    if (args.generate_config):
        ...
        if not (os.path.isdir(config_path)):
            try:
                ...
                csvfile.write(
                    "Generated by sysinfo-snapshot version " +
                    version +
                    " \n")
                fieldnames = [
                    COMMAND_CSV_HEADER,
                    INVOKED_CSV_HEADER,
                    FLAG_RELATED_HEADER]
                config_file = csv.DictWriter(
                    csvfile, fieldnames=fieldnames, lineterminator="\n")
                if (sys.version_info[0] == 2 and sys.version_info[1] < 7):
                    config_file.writerow(
                        {
                            COMMAND_CSV_HEADER: COMMAND_CSV_HEADER,
    ...
    if (args.mtusb):
        mtusb_flag = True
        # Change Name: mst_commands_query_output --> mst_commands_query_output
        # / i2c-mst_commands_query_output
        try:
            commands_collection.remove('mst_commands_query_output')
        except BaseException:
            pass
        commands_collection.extend(
            ['mst_commands_query_output / i2c-mst_commands_query_output'])
        try:
            fw_collection.remove('mst_commands_query_output')
        except BaseException:
            pass
        fw_collection.extend(
            ['mst_commands_query_output / i2c-mst_commands_query_output'])
...
def get_parsed_args():
    ...
    if (sys.version_info[0] == 2 and sys.version_info[1] < 7):
        ...
        parser.add_option(
            ...
            help="set destination directory (default is /tmp/).")
        parser.add_option(
            "-v",
            "--version",
            help="show the tool's version information and exit.",
            action='store_true')
        parser.add_option(
            "-p",
            "--perf",
            help="include more performance commands/functions, e.g. ib_write_bw and ib_write_lat.",
            action='store_true')
        parser.add_option(
            "--ufm",
            help="add ufm logs to the output.",
            action='store_true')
        parser.add_option(
            "--no_fw",
            help="do not add firmware commands to the output.",
            action='store_true')
        parser.add_option(
            "--fsdump",
            help="add fsdump firmware command to the output.",
            action='store_true')
        parser.add_option(
            "--mtusb",
            help="add I2C mstdump files to the output.",
            action='store_true')
        parser.add_option(
            "--ibdiagnet",
            help="add ibdiagnet command to the output.",
            action='store_true')
        parser.add_option(
            "--ibdiagnet_ext",
            help="add ibdiagnet ext command to the output.",
            action='store_true')
        parser.add_option(
            "--with_inband",
            help="add in-band cable info to the output.",
            action='store_true')
        parser.add_option(
            "--no_ib",
            help="do not add server IB commands to the output.",
            action='store_true')
        parser.add_option(
            "--openstack",
            help="gather openstack relevant conf and log files",
            action='store_true')
        parser.add_option(
            "--asap",
            help="gather asap relevant commands output",
            action='store_true')
        parser.add_option(
            "--asap_tc",
            help="gather asap tc filter commands output",
            action='store_true')
        parser.add_option(
            "--rdma_debug",
            help="gather rdma tool that comes with iproute2 commands output",
            action='store_true')
        parser.add_option(
            "--gpu",
            help="gather Nvidia GPU commands",
            action='store_true')
        parser.add_option(
            "--json",
            help="add json file to the output.",
            action='store_true')
        parser.add_option(
            "--pcie",
            help="add pcie commands/functions to the output.",
            action='store_true')
        parser.add_option(
            "--pcie_debug",
            help="generate only pcie debug info.",
            action='store_true')
        parser.add_option(
            "--config",
            dest="config",
            ...
            help="set the customized configuration file path including filename, to choose which commands are approved to run.\n" +
        ...
        parser.add_option(
            ...
            action="callback",
            callback=config_callback,
            help="Generates configuration file under provided path,Path must be full path, including file name.\n" +
        ...
        parser.add_option(
            "--check_fw",
            help="check if the current adapter firmware is the latest version released, output in performance html file [Internet access is required]",
            action='store_true')
        parser.add_option(
            "--verbose",
            help="first verbosity level, available if option is provided only once, lists sections in process.second verbosity level, available if option is provided twice, lists sections and commands in process.",
            action='count')
        parser.add_option(
            "--non_root",
            help=" Allow the tool to run as non_root.",
            action='store_true')
        parser.add_argument(
            "-t",
            "--nvsm_dump",
            help=" collect nvsm dump health.",
            action='store_true')
        (options, args) = parser.parse_args()

        if (len(args) > 0) and (not (options.dir) and not (
                options.config) and not (options.generate_config)):
        ...
        parser.add_argument(
            ...
            help="set the customized configuration file path including the filename, to choose which commands are approved to run.\n" +
            "In case a path is not provided, the default file name(config.csv) and it path are set for the same directory.")
        parser.add_argument(
            "--generate_config",
            nargs="?",
        ...
        parser.add_argument(
            "--non_root",
            help=" allow the tool to run as non-root.",
            action='store_true')
        parser.add_argument(
            "-t",
            "--nvsm_dump",
