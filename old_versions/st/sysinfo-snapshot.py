...
internal_files_collection = ["/etc/infiniband/info", "/etc/infiniband/openib.conf", "/etc/modprobe.d/vxlan.conf", "/etc/security/limits.conf", "/boot/grub/grub.cfg", "/boot/grub/grub.conf", "/boot/grub/menu.lst", "/etc/default/grub", "/etc/host.conf", "/etc/hosts", "/etc/hosts.allow", "/etc/hosts.deny", "/etc/issue", "/etc/modprobe.conf", "/etc/ntp.conf", "/etc/resolv.conf", "/etc/sysctl.conf", "/etc/tuned.conf", "/etc/yum.conf", "/proc/cmdline", "/proc/cpuinfo", "/proc/devices", "/proc/diskstats", "/proc/dma", "/proc/interrupts", "/proc/meminfo", "/proc/modules", "/proc/mounts", "/proc/net/dev_mcast", "/proc/net/igmp", "/proc/partitions", "/proc/stat", "/proc/sys/net/ipv4/igmp_max_memberships", "/proc/sys/net/ipv4/igmp_max_msf", "/proc/uptime", "/proc/version"]
...
def fw_ini_dump_handler():
	st, res = commands.getstatusoutput(
        ...
        res2 = ""
        if mtusb_flag:
                st3, mst_start = commands.getstatusoutput("mst start")
                if st3 != 0:
...
def lspci_xxxvvv_handler():
	...
	st, res = commands.getstatusoutput(
                                "for interface in `lspci " + all_hca + " | awk '{print $1}'`; " +
                                "do " +
...
def ib_find_bad_ports_handler():
	...
	for line in ibnetdiscover_p:
		...
		sta, ibportstate_G = commands.getstatusoutput("/usr/sbin/ibportsate -F " + GUID + " " + PORT + " > " + path+file_name + "/ibportstate_g")
		...
		st3 = 1
		if (sta == 0):
			st1, ACTIVE_WIDTH = commands.getstatusoutput("cat " + path + file_name + "/ibportstate_g | grep LinkWidthActive | head -1 | sed 's/.\.\./ /g' | awk '{print $(NF)}'" )	
			st2, ACTIVE_SPEED = commands.getstatusoutput("cat " + path + file_name + "/ibportstate_g | grep LinkSpeedActive | head -1 | sed 's/.\.\./ /g' | awk '{print $2}'")
			st3, ENABLE_SPEED = commands.getstatusoutput("cat " + path + file_name + "/ibportstate_g | grep LinkSpeedEnabled |head -1 | sed 's/\.\./ /g' | awk '{print $(NF-1)}'")
		...
		if (st2 == 0 and st3 == 0 and ACTIVE_SPEED != ENABLE_SPEED):
			st, PEER_ENABLED_SPEED = commands.getstatusoutput("cat " + path + file_name + "/ibportstate_g | grep LinkSpeedEnabled |tail -1| sed 's/\.\./ /g' | awk '{print $(NF-1)}'")
	...
	invoke_command(['rm', '-rf', path+file_name+"/ibportstate_g"])
...
def show_usage():
	print ("sysinfo-snapshot version: " + version + " usage:"
	  ...
	  + "\n\t--mtusb \t\t- add I2C mstdump files to the output. (available only if -fw option was provided)."
...
def bw_and_lat():
	...
	for device in devices:
		...
		latency[device] = "Not invoking 'ib_write_lat -d " + device + " >/dev/null & ib_write_lat -d " + device + " localhost'\nReason: It may hang"
...
def update_flags():
	...
	w = 1
		...
		if (arg == "--mtusb"):
			...
			w += 1
		if (i>2 or j>2 or k>2 or l>2 or z>2 or w>2):
