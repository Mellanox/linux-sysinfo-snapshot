...
internal_files_collection = ["/etc/security/limits.conf", "/boot/grub/grub.cfg", "/boot/grub/grub.conf", "/boot/grub/menu.lst", "/etc/default/grub", "/etc/host.conf", "/etc/hosts", "/etc/hosts.allow", "/etc/hosts.deny", "/etc/issue", "/etc/modprobe.conf", "/etc/ntp.conf", "/etc/resolv.conf", "/etc/sysctl.conf", "/etc/tuned.conf", "/etc/yum.conf", "/proc/cmdline", "/proc/cpuinfo", "/proc/devices", "/proc/diskstats", "/proc/dma", "/proc/interrupts", "/proc/meminfo", "/proc/modules", "/proc/mounts", "/proc/net/dev_mcast", "/proc/net/igmp", "/proc/partitions", "/proc/stat", "/proc/sys/net/ipv4/igmp_max_memberships", "/proc/sys/net/ipv4/igmp_max_msf", "/proc/uptime", "/proc/version"]
...
external_files_collection = [["kernel config", "/boot/config-$(uname -r)"], ["config.gz", "/proc/config.gz"], ["dmesg", "dmesg"], ["biosdecode", "biosdecode"], ["dmidecode", "dmidecode"], ["syslog", "/var/log/"], ["libvma.conf", "/etc/libvma.conf"], ["ibnetdiscover", ""], ["Installed packages", ""], ["mlnx_tune", "mlnx_tune --dump_status"]]
...
def ib_topology_viewer_handler():
	...
	if (st != 0):
		return "Could not run: " + '"' + "ibnetdiscover -p | grep -v -i sfb | grep -e ^SW | awk '{print $4}' | uniq" + '"'
	if (GUIDS == ""):
...
def add_external_file_if_exists(field_name, curr_path):
	...
	elif (field_name == "mlnx_tune"):
		status, command_output = commands.getstatusoutput(curr_path)
		if (status == 0 and command_output != "" and not "command not found" in command_output):
			add_ext_file_handler(curr_path, field_name, command_output)
		else:
			status, command_output = commands.getstatusoutput("./" + curr_path)
			if (status == 0 and command_output != "" and not "No such file or directory" in command_output):
				add_ext_file_handler(curr_path, field_name, command_output)
			else:
				err_flag = 1
				err_command += curr_path
	else:	
...
def generate_output():	
	...
	if (verbose_flag == True):
		print ("------------------------------------------------------------\n")
		print ("Generating HTML page has started")	
	...
	if (verbose_flag == True):
		print ("Generating HTML page has ended\n")
