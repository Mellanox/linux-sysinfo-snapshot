...
internal_files_collection = ["/etc/modprobe.d/vxlan.conf", "/etc/security/limits.conf", "/boot/grub/grub.cfg", "/boot/grub/grub.conf", "/boot/grub/menu.lst", "/etc/default/grub", "/etc/host.conf", "/etc/hosts", "/etc/hosts.allow", "/etc/hosts.deny", "/etc/issue", "/etc/modprobe.conf", "/etc/ntp.conf", "/etc/resolv.conf", "/etc/sysctl.conf", "/etc/tuned.conf", "/etc/yum.conf", "/proc/cmdline", "/proc/cpuinfo", "/proc/devices", "/proc/diskstats", "/proc/dma", "/proc/interrupts", "/proc/meminfo", "/proc/modules", "/proc/mounts", "/proc/net/dev_mcast", "/proc/net/igmp", "/proc/partitions", "/proc/stat", "/proc/sys/net/ipv4/igmp_max_memberships", "/proc/sys/net/ipv4/igmp_max_msf", "/proc/uptime", "/proc/version"]
...
external_files_collection = [["kernel config", "/boot/config-$(uname -r)"], ["config.gz", "/proc/config.gz"], ["dmesg", "dmesg"], ["biosdecode", "biosdecode"], ["dmidecode", "dmidecode"], ["syslog", "/var/log/"], ["libvma.conf", "/etc/libvma.conf"], ["ibnetdiscover", ""], ["Installed packages", ""], ["Performance tuning analyze", ""]]
...
def eth_tool_all_interfaces_handler():
	...
	first = True
	res = ""
	options = ["", "-i", "-g", "-a", "-k", "-c", "-T", "--show-priv-flags", "-n", "-l"]
	for interface in net_devices:
		...
		if (st != 0):
			ethtool_interface = "Could not run command: ethtol -S " + interface
...
def add_command_if_exists(command):
	...
	if (command == "date"):
		...
		if (st == 0):	
			...
			status = 1
			print_err_flag = 1
			result = "Could not run: " + '"' + " awk '{ print FILENAME " + '"' + "=" + '"' + " $0  }' /sys/module/mlx4_*/parameters/* " + '"'
		...
		if (status == 0):
			...
			status = 1
			print_err_flag = 1
			result = "Could not run: " + '"' + " modprobe bonding; find /proc/net/bonding/ |xargs grep ^ " + '"'
		...
		if (status == 0):	
			...
			status = 1
			print_err_flag = 1
			result = "Could not run: " + '"' + " modprobe bonding; find /sys/class/net/ |xargs grep ^ " + '"'
	else:
...
def multicast_information_handler():
	st_saquery, SAQUERY = commands.getstatusoutput("`which saquery 2>/dev/null`")
        if (st_saquery != 0):
      		return "`which saquery`  command is not found"

	st, saquery_g = commands.getstatusoutput("`which saquery 2>/dev/null` -g")
	if (st != 0):
		return "`which saquery` -g  command is not found"
	...
	MLIDS = MLIDS.split()	
        ...
        for MLID in MLIDS:
                st, saquery_mlid = commands.getstatusoutput("SAQUERY=`which saquery 2>/dev/null`; ${SAQUERY} -m " + MLID + " --smkey 1")
                if (st != 0):
			saquery_mlid = "Could not run: " + '"' + "SAQUERY=`which saquery 2>/dev/null`; ${SAQUERY} -m " + MLID + " --smkey 1" + '"'
...
def ib_find_bad_ports_handler():
	...
	for line in ibnetdiscover_p:
		...
		st1, ACTIVE_WIDTH = commands.getstatusoutput("/usr/sbin/ibportstate -G " + GUID + " " + PORT + " | grep LinkWidthActive | head -1 | sed 's/.\.\./ /g' | awk '{print $(NF)}'" )	
		st2, ACTIVE_SPEED = commands.getstatusoutput("/usr/sbin/ibportstate -G " + GUID + " " + PORT + " | grep LinkSpeedActive | head -1 | sed 's/.\.\./ /g' | awk '{print $2}'")
		st3, ENABLE_SPEED = commands.getstatusoutput("/usr/sbin/ibportstate -G " + GUID + " " + PORT + " | grep LinkSpeedEnabled |head -1 | sed 's/\.\./ /g' | awk '{print $(NF-1)}'")
		
		if (st1 == 0 and ACTIVE_WIDTH == "1X"):
			count_1x += 1	
			res += "GUID:" + GUID + " PORT:" + PORT + " run in 1X width\n"
		
		if (st2 == 0 and st3 == 0 and ACTIVE_SPEED != ENABLE_SPEED):
			...
			if (st == 0 and ACTIVE_SPEED != PEER_ENABLED_SPEED):
				count_deg += 1
...
def ib_find_disabled_ports_handler():	
	...
	for line in ibnetdiscover_p:
		...
		if (st == 0 and LINK_STATE == "Disabled"):
			...
			if (st1 == 0):
				...
				for row in res1:
					if (row.lower().startswith("switch") == False):
						continue
					res2 += row + "\n"
				#st, res = commands.getstatusoutput("echo " + res2 + " | grep " + GUID + " | grep -q sRB-20210G-1UP 2>/dev/null")
				st, res = commands.getstatusoutput("/usr/sbin/ibswitches | grep -i " + '"' + "^switch" + '"' + " | grep " + GUID + " | grep -q sRB-20210G-1UP")
...
def ib_mc_info_show_handler():
	...
	st, saquery = commands.getstatusoutput("SAQUERY=`which saquery 2>/dev/null`; ${SAQUERY} -m --smkey 1")
	if (st != 0):
		return "Could not run: " + '"' + "SAQUERY=`which saquery 2>/dev/null`; ${SAQUERY} -m --smkey 1" + '"'
...
def sm_master_is_handler():	
	...
	if (st != 0):
		return "Could not retrieve all SM. Reason: Could not run " + '"' + "SAQUERY=`which saquery 2>/dev/null`; ${SAQUERY} -s |grep base_lid |head -1| sed 's/\./ /g'|awk '{print $2}'" + '"'
	SMS = set(SMS.split())
...
def sm_status_handler():	
	...
	for lo in range(0,4): 
		...
		if (SmActivity != "<N/A>" and represents_Int(SmActivity) == True):	
...
def ibdiagnet_handler():
	if (ibdiagnet_is_invoked == False):
		...
		#if (st != 0):
		#	ibdiagnet_res = "Couldn't find command: ibdiagnet -o"
...
def add_fabric_command_if_exists(command):
	...
	if (command == "Multicast_Information"):
		...
		if (status == 0 and result == "" and (command == "ibhosts" or "ibswitches")):
			result = "There are no " + command
...
def add_ext_file_handler(field_name, fil_name, command_output):
	if ( fil_name != "pkglist" and (not "erformance" in fil_name)):
...
def add_external_file_if_exists(field_name, curr_path):
	...
	if (field_name == "kernel config"):
		...
		status, command_output = commands.getstatusoutput("./mlnx_tune -r")
	...
	if (err_flag == 1):
		f = open(path+file_name+"/dummy_external_paths", 'a')
		f.write(err_command)
		f.write("\n\n")
		f.close()
	...
	if (os.path.exists(path + file_name) == True and os.path.isfile(path + file_name) == False):
		...
		for fi in sorted(os.listdir(path + file_name)):
			if (fi == "dummy_functions"):
				...
				print ("ethtool_S \t\t- contains all files generated from invoking ethtool -S <interface>")
			else:
				print (fi)
...
def show_usage():
	print ("sysinfo-snapshot version: " + version + " usage:"
	  ...
	  + "\n\t--verbose\t\t- verbosely list sections in process.")
...
def hyper_threading():
	...
	if (st != 0):
		...
		if (st == 0 and int(siblings_lines_num)==0):
			siblings = ""
		else:
	...
	if (st != 0):
		...
		if (st == 0 and int(cpu_cores_lines_num)==0):
			cpu_cores = ""
		else:
...
def bw_and_lat():
	...
	for device in devices:
		bandwidth[device] = ""
		st, bandwidth[device] = commands.getstatusoutput("ib_write_bw --report_gbits -d " + device + " >/dev/null & ib_write_bw --report_gbits -d " + device + " localhost")
		latency[device] = ""
		st, latency[device] = commands.getstatusoutput("ib_write_lat -d " + device + " >/dev/null & ib_write_lat -d " + device + " localhost")
...
def confirm_mlnx_cards():
	...
	st, count = commands.getstatusoutput("lspci -d 15b3: | wc -l")
	if (st != 0 or ("command not found" in count)):
		st, count = commands.getstatusoutput("ls /sys/class/infiniband")
		if (st == 0 and ("No such file or directory" not in count)):
			mlnx_cards_status = len(count.split())
	else:
		count = (count.splitlines())[-1]
        	if (represents_Int(count) == True):
			mlnx_cards_status = int(count)
